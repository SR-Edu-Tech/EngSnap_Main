...
public class U9_R02_Junior1A : MonoBehaviour, Interfaces_Junior1A
{
    ...
    void OnDisable()
    {
        foreach (string clickId in _clickCheckIndex)
        {
            ...
            Image img = tabP.GetChild(0).GetChild(btnIndex).GetChild(1).GetComponent<Image>();
    ...
    public void PlayAudio(int index)
    {
        ...
        if (!_clickCheckIndex.Contains(clickId))
        {
            ...
            Image img = currentTabP.GetChild(0).GetChild(index).GetChild(1).GetComponent<Image>();
            Color c = img.color;
            c.r *= .85f;
            c.g *= .85f;
            c.b *= .85f;
