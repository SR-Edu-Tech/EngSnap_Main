...
[Serializable]
public class U8_Q01_Junior1A_QuestionData
{
...
public class U8_Q01_Junior1A : MonoBehaviour, Interfaces_Junior1A
{
    [SerializeField] AudioSource _audioSource;
    [SerializeField] AudioClip _introClip, _wrongClip, _correctClip;
    [SerializeField] U8_Q01_Junior1A_QuestionData[] _questions;
    [SerializeField] GameObject _headingObj, _buttonParent, _object, _imageObj, _completed, _mainHeading;
    ...
    IEnumerator CheckAnswer(int index)
    {
        if (index == _questions[_currentQuestionIndex].CorrectAnsIndex)
        {
            ...
            _buttonParent.transform.GetChild(index).GetComponent<PopEffect_Junior1A>().enabled = false;
            _buttonParent.transform.GetChild(index).GetComponent<PopEffect_Junior1A>().enabled = true;
        ...
        else
        {
            ...
            _buttonParent.transform.GetChild(index).GetComponent<WiggleEffect_Junior1A1>().enabled = true;
        ...
        if (_currentQuestionIndex < _questions.Length - 1)
        {
            ...
            {
                obj.GetChild(0).GetComponent<TextMeshProUGUI>().text = _questions[_currentQuestionIndex].OptionText[_currentOptionIndex];
                obj.GetComponent<PopEffect_Junior1A>().enabled = true;
                _currentOptionIndex++;
            }
            ...
            _headingObj.SetActive(false);
            _headingObj.SetActive(true);
            if (_currentQuestionIndex == 3) _imageObj.SetActive(true);
        ...
        else
        {
            ...
            for (int i = 0; i < _questions.Length; i++)
            {
                if (i < _starWon)
                {
                    ...
                    _completed.transform.GetChild(i).GetComponent<PopEffect_Junior1A>().enabled = true;
                    _audioSource.clip = _correctClip;
                    _audioSource.Play();
