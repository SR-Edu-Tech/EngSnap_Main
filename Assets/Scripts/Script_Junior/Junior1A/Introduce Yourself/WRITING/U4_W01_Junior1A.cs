...
[Serializable]
public class U4_W01_Junior1A_QuestionData
{
...
public class U4_W01_Junior1A : MonoBehaviour, Interfaces_Junior1A
{
    ...
    [SerializeField] U4_W01_Junior1A_QuestionData[] _questionData;
    ...
    IEnumerator CheckDataValidity()
    {
        if (_currentAnswerText == _questionData[_currentQuestionIndex].AnswerText)
        {
            ...
            if (_currentQuestionIndex < _questionData.Length)
            {
                ...
                foreach(Transform option in _spawnBox) option.GetComponent<Button>().interactable = true;
            }
            else
            {
