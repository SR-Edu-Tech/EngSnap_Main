...
[Serializable]
public class U11_G01_Junior1A_QuestionData
{
...
public class U11_G01_Junior1A : MonoBehaviour, Interfaces_Junior1A
{
    ...
    [SerializeField] U11_G01_Junior1A_QuestionData[] _questionData; // Fixed class type match naming
    ...
    IEnumerator Starter()
    {
        ...
        if (_completionTextObject != null) _completionTextObject.SetActive(false); // Hide text initially
    ...
    {
        foreach (U11_G01_Junior1A_QuestionData item in _questionData)
        {
            _fish.GetComponent<SlideEffect_Junior1A>().enabled = false;
            _fish.GetComponent<RectTransform>().anchoredPosition = new Vector2(0, -25);
            _fish.GetComponent<SlideEffect_Junior1A>()._targetPosition = new Vector3(0, -25, 0);
            ...
            _fish.GetComponent<SlideEffect_Junior1A>()._targetPosition = new Vector3(-1600, -25, 0);
            _fish.GetComponent<SlideEffect_Junior1A>().enabled = true;
            foreach (Transform button in _buttonParent.transform) button.gameObject.SetActive(false);
        ...
        // 1. Let all stars fully spawn in one by one first
        ...
        // 2. NOW that all stars are fully processed, activate and pop the text object
        if (_completionTextObject != null)
        {
            _completionTextObject.SetActive(true);
            
            // If the text object has a PopEffect attached, trigger it instantly
            ...
            // Brief visual pause to admire the score screen layout cleanly
            yield return new WaitForSeconds(1.5f);
        }

        // 3. Complete and allow transition onward
