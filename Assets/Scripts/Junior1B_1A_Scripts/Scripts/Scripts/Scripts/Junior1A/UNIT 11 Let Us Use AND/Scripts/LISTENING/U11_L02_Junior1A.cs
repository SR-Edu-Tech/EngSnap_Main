...
public class U11_L02_Junior1A : MonoBehaviour, Interfaces_Junior1A
{
    ...
    private IEnumerator Starter()
    {
        ...
        // Reset the background layout colors on starter loop initializations safely
        ...
        foreach (Transform button in _cardParent)
        {
            ...
            if (button.TryGetComponent(out PopEffect_Junior1A pop)) 
            {
    ...
    private IEnumerator RepeatAudio()
    {
        ...
        foreach (Transform button in _cardParent)
        {
            if (button.TryGetComponent(out PopEffect_Junior1A pop)) pop.enabled = true;
