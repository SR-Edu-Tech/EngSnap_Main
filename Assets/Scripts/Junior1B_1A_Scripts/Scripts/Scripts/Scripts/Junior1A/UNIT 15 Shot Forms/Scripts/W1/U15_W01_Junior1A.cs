...
[Serializable]
public class U15_W01_Junior1A_QuestionData
{
...
public class U15_W01_Junior1A : MonoBehaviour, Interfaces_Junior1A
{
    ...
    [SerializeField] private U15_W01_Junior1A_QuestionData[] _questionData;
    ...
    private IEnumerator CheckDataValidity()
    {
        ...
        bool isEntireSentenceCorrect = true;

        // 1. First Pass: Read and evaluate if there are any errors across the selection layout
        int checkIndex = 0;
        ...
        // 2. Branch Logic Processing Evaluation Verdicts
        if (isEntireSentenceCorrect)
        {
            _checkBox.gameObject.SetActive(false);
        ...
        {
            // Execution block handling player error layouts
            ...
            // Revert colors so player can rearrange their chosen cards
            foreach (Transform option in _answerBox)
            {
                if (option.TryGetComponent(out Image img)) img.color = Color.white;
            }
