...
public class U14_SP01_Junior1A : MonoBehaviour, Interfaces_Junior1A
{
    ...
    [SerializeField] private string _talkingBoolName = "IsListening";
    ...
    IEnumerator Starter()
    {
        ...
        // 🛑 PHASE 1: Turn on Titles only. Explicitly lock down the gameplay elements.
        ...
        // 🚀 PHASE 2: Intro audio complete! Unveil everything together perfectly.
        ShowTargetWord();
    }
   ...
   public void OnLocalNextButtonPressed()
    {
        ...
        // 3. Local Layout Check: If we have more questions left, move to the next index
        ...
        {
            // 4. End of Task: Disable the microphone safely
            if (_micObj != null) _micObj.GetComponent<Button>().interactable = false;
            _isViewed = true;
            
            // 5. Exit back to topic select without touching the global next layout logic
            if (GameManager_Junior1A.Instance != null)
            {
                GameManager_Junior1A.Instance.NextSlide();
            }
        }
    }
    ...
    IEnumerator AudioChecker(bool isMatch)
    {
        if (isMatch)
        {
            ...
            // Pop up manual next button control step instead of auto-running indices
