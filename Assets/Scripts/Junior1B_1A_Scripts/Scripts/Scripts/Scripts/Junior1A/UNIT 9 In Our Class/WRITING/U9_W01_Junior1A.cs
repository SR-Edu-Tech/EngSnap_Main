...
[Serializable]
public class U9_W01_Junior1A_QuestionData
{
...
public class U9_W01_Junior1A : MonoBehaviour, Interfaces_Junior1A
{
    ...
    [SerializeField] U9_W01_Junior1A_QuestionData[] _questionData;
    ...
    IEnumerator Starter()
    {
        ...
        foreach (string data in _questionData[_currentQuestionIndex].OptionText)
        {
            ...
            var btn = _spawnBox.GetChild(_currentAnswerIndex).GetComponent<Button>();
            if (btn) btn.interactable = false;
            _currentAnswerIndex++;
            LayoutRebuilder.ForceRebuildLayoutImmediate(_spawnBox as RectTransform);
            yield return new WaitForSeconds(0.5f);
        }
        foreach (Transform child in _spawnBox) if (child.GetComponent<Button>()) child.GetComponent<Button>().interactable = true;
    ...
    IEnumerator CheckDataValidity()
    {
        if (_currentAnswerText == _questionData[_currentQuestionIndex].AnswerText)
        {
            ...
            if (_currentQuestionIndex < _questionData.Length)
            {
                ...
                foreach (string data in _questionData[_currentQuestionIndex].OptionText)
                {
                    var slot = _spawnBox.GetChild(_currentAnswerIndex);
                    slot.GetChild(0).GetComponent<TextMeshProUGUI>().text = data;
                    slot.gameObject.SetActive(true);
                    var btn = slot.GetComponent<Button>();
                    if (btn) btn.interactable = false;
                    slot.GetComponent<PopEffect_Junior1A>().enabled = true;
                    _currentAnswerIndex++;
                    LayoutRebuilder.ForceRebuildLayoutImmediate(_spawnBox as RectTransform);
                    yield return new WaitForSeconds(0.5f);
                }
                foreach (Transform childSlot in _spawnBox) if (childSlot.GetComponent<Button>()) childSlot.GetComponent<Button>().interactable = true;
                _checkBox.GetComponent<Button>().interactable = true;
            }
            else
            {
