...
public class U6_L02_Junior1B : MonoBehaviour, Interfaces_Junior1B
{
    ...
    void OnEnable()
    {
        ...
        // 1. Keep the entire Tab_1Data container hidden on startup while intro plays
    ...
    {
        // 2. Play introduction voice track first while screen is clean
        ...
        // 3. Intro finished! Turn on Tab_1Data completely
        if (_dialogueContainerParent != null)
        {
            _dialogueContainerParent.SetActive(true);
            
            // Force CanvasGroup alpha to full visibility just in case
        ...
        // 4. Ensure all nested dialogues are active and reset scroll positioning
        if (_contentGrid != null)
        {
            ...
            // Snap the scroll container frame back to the very top boundary
            _contentGrid.GetComponent<RectTransform>().anchoredPosition = Vector2.zero;
        }

        yield return new WaitForSeconds(0.3f);
        
        // 5. Fire the sequential A -> B -> Input autoplay engine
    ...
    {     
        ...
        // Keep manual clicking disabled while autoplay sequence is operating
        ...
        foreach (Transform child in _contentGrid)
        {
            ...
            {
                // Programmatically click the student or teacher button line
                btn.onClick.Invoke();
                ...
                // FLOW CONTROL TRIGGER: If it's a Teacher line (Odd index rows), halt and display input box
                if (child.name.Contains("Teacher") || currentLoopIndex % 2 != 0)
                {
                    OpenPlayerInputInterface(currentLoopIndex);
                    
                    // Stay paused here until player hits submit
        ...
        // Re-enable direct user interactions post-autoplay completion
        foreach (Transform child in _contentGrid)
        {
