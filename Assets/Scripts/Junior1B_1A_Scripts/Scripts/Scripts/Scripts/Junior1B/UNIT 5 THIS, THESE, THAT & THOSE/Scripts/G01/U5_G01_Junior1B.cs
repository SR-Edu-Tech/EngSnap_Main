...
[Serializable]
public class U5_G01_Junior1B_QuestionData
{
...
public class U5_G01_Junior1B : MonoBehaviour, Interfaces_Junior1B
{
    ...
    [SerializeField] U5_G01_Junior1B_QuestionData[] _questionData;
    ...
    IEnumerator MoveFish()
    {
        ...
        foreach (U5_G01_Junior1B_QuestionData item in _questionData)
        {
            if (item == null) continue;

            // Reset Slide component transitions safely
            if (_fish.TryGetComponent(out SlideEffect_Junior1B slide))
            {
                slide.enabled = false;
            ...
            foreach (Transform button in _buttonParent.transform)
            {
                ...
                if (button.childCount > 0 && button.GetChild(0).TryGetComponent(out TextMeshProUGUI btnTxt))
                {
                    ...
                    if (button.GetChild(0).TryGetComponent(out TextPopEffect_Junior1A textPop))
                    {
                ...
                if (button.TryGetComponent(out Popeffect_Junior1B pop)) pop.enabled = true;
                ...
                _selectedButton == _buttonParent.transform.GetChild(item.CorrectAnsIndex))
            {
                ...
                if (_selectedButton.TryGetComponent(out Popeffect_Junior1B pop)) pop.enabled = true;
            }
            else
            {
            ...
            if (_fish.TryGetComponent(out SlideEffect_Junior1B slideOut))
            {
        ...
        if (_starsPanel != null)
        {
            ...
            for (int i = 0; i < _starsPanel.transform.childCount - 1; i++)
            {
                ...
                if (star.TryGetComponent(out Popeffect_Junior1B pop)) pop.enabled = true;
