...
[Serializable]
public class U8_W01_Junior2B_QuestionData
{
...
public class U8_W01_Junior2B : MonoBehaviour, Interfaces_Junior2B
{
    ...
    [SerializeField] U8_W01_Junior2B_QuestionData[] _questionData;
    ...
    IEnumerator SpawnOptionsRoutine()
    {
        ...
        for (int i = 0; i < currentOptions.Length; i++)
        {
            ...
            if (choiceButton.TryGetComponent(out Popeffect_Junior2B pop))
            {
                pop.enabled = false;
                pop.enabled = true;
            }
    ...
    IEnumerator CheckDataValidity()
    {
        ...
        for (int i = 0; i < _answerBox.childCount; i++)
        {
            ...
            if (child.TryGetComponent(out Image buttonImage) && child.childCount > 0 && child.GetChild(0).TryGetComponent(out TextMeshProUGUI letterText))
            {
                ...
                {
                    buttonImage.color = Color.red; // Correct slot choice -> Turn Box Green
                }
                else
                {
                    buttonImage.color = Color.green; // Mistaken slot choice -> Turn Box Red
                }
            }
        }
        ...
        else
        {
            ...
            if (_answerBox.TryGetComponent(out WiggleEffect_Junior2B wiggle))
            {
