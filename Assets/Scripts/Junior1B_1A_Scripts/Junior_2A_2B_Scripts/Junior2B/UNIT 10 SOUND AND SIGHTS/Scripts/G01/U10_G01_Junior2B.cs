...
[Serializable]
public class U10_G01_Junior2B_QuestionData
{
...
public class U10_G01_Junior2B : MonoBehaviour, Interfaces_Junior2B
{
    ...
    [SerializeField] U10_G01_Junior2B_QuestionData[] _questionData;
    ...
    {
        foreach (U10_G01_Junior2B_QuestionData item in _questionData)
        {
            _fish.GetComponent<SlideEffect_Junior2B>().enabled = false;
            _fish.GetComponent<RectTransform>().anchoredPosition = new Vector2(0, -25);
            _fish.GetComponent<SlideEffect_Junior2B>()._targetPosition = new Vector3(0, -25, 0);
            _fish.GetComponent<SlideEffect_Junior2B>().enabled = true;
            ...
            foreach (Transform button in _buttonParent.transform)
            {
                ...
                button.GetComponent<Popeffect_Junior2B>().enabled = true;
                button.gameObject.SetActive(true);
                button.GetChild(0).GetComponent<TextPopEffect_Junior2B>().enabled = false;
                button.GetChild(0).GetComponent<TextPopEffect_Junior2B>().enabled = true;
            }
            ...
            if (item.CorrectAnsIndex < _buttonParent.transform.childCount && _selectedButton == _buttonParent.transform.GetChild(item.CorrectAnsIndex))
            {
                ...
                _selectedButton.GetComponent<Popeffect_Junior2B>().enabled = true;
            }
            else
            {
                ...
                _selectedButton.GetComponent<WiggleEffect_Junior2B>().enabled = true;
            }
            ...
            _fish.GetComponent<SlideEffect_Junior2B>()._targetPosition = new Vector3(-1600, -25, 0);
            _fish.GetComponent<SlideEffect_Junior2B>().enabled = true;
            foreach (Transform button in _buttonParent.transform) button.gameObject.SetActive(false);
        ...
        {
            _starsPanel.transform.GetChild(i).GetComponent<Popeffect_Junior2B>().enabled = true;
        ...
        if (_completionTextObject != null)
        {
            ...
            if (_completionTextObject.TryGetComponent(out Popeffect_Junior2B textPop))
            {
