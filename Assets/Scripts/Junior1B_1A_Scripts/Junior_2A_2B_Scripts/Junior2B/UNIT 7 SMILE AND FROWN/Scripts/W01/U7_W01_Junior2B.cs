...
[Serializable]
public class U7_W01_Junior2B_QuestionData
{
...
public class U7_W01_Junior2B : MonoBehaviour, Interfaces_Junior2B
{
    ...
    [SerializeField] U7_W01_Junior2B_QuestionData[] _questionData;
    ...
    private void ResetBoxes()
    {
        ...
        foreach (Transform child in _spawnBox)
        {
            ...
            child.localScale = Vector3.one;
            if (child.TryGetComponent(out Popeffect_Junior2B pop)) pop.enabled = true;
        }
    }
    ...
    private void SpawnItem(string displayContent)
    {
        ...
        if (item.TryGetComponent(out Popeffect_Junior2B pop)) pop.enabled = true;
    ...
    IEnumerator CheckDataValidity()
    {
        ...
        if (string.Equals(processedAnswer, processedTarget, StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (_answerBox.TryGetComponent(out Popeffect_Junior2B pop))
            {
                pop.enabled = false;
                pop.enabled = true;
            }
        ...
        else
        {
            ...
            if (GetComponent<WiggleEffect_Junior2B>() != null) GetComponent<WiggleEffect_Junior2B>().enabled = true;
            else if (_answerBox.GetComponent<WiggleEffect_Junior2B>() != null) _answerBox.GetComponent<WiggleEffect_Junior2B>().enabled = true;
