...
namespace Unit5.R02.Junior2B
{
    public class U6_R02_Junior2B : MonoBehaviour, Interfaces_Junior2B
    {
        ...
        private Transform FindButtonByIndex(int index)
        {
            ...
            int tab2ButtonCount = _tab1Content.childCount;
            if (index < tab2ButtonCount)
            {
                return (index >= 0 && index < tab2ButtonCount) ? _tab1Content.GetChild(index) : null;
            }
            else
            {
