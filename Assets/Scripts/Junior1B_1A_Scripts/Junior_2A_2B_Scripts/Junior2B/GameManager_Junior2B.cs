...
namespace Junior2B
{
    ...
    public class GameManager_Junior2B : MonoBehaviour
    {
    public static GameManager_Junior2B Instance;
    ...
    [Header("Console Debugging (Optional)")]
    ...
    public void SelectLesson(int lessonIndex)
    {
        ...
        if (_selectedLessonIndex >= 0 && _selectedLessonIndex < _lessons.Length)
        {
            ...
            {
                Transform topicNode = _topicParent.transform.GetChild(0).GetChild(0).GetChild(currentTopicIndex);
                if (topic.IsCompleted)
                {
                    topicNode.GetChild(1).GetComponent<Image>().enabled = true;
                }
                else
                {
                    topicNode.GetChild(1).GetComponent<Image>().enabled = false;
                }
                currentTopicIndex++;
            }
        }
    ...
    public void SelectTopic(int topicEnumIndex)
    {
        ...
        if (_currentTopicData != null && _currentTopicData.Slides != null && _currentTopicData.Slides.Length > 0)
        {
            ...
            {
                Interfaces_Junior2B slideInterface = _currentTopicData.Slides[i].GetComponent<Interfaces_Junior2B>();
                if (slideInterface != null && slideInterface.IsViewed)
                {
            ...
            if (_currentSlideIndex >= _currentTopicData.Slides.Length)
            {
                _currentSlideIndex = 0;
            }
    ...
    {
        if (_currentTopicData == null || _currentSlideIndex >= _currentTopicData.Slides.Length) return;
    ...
    public void Back()
    {
        ...
        if (_currentSlideIndex < 0 || _currentTopicData == null)
        {
            ...
            for (int i = 0; i < _lessonParent.transform.childCount; i++)
            {
                var pop = _lessonParent.transform.GetChild(_lessonParent.transform.childCount - (1 + i))
                          .GetComponent<Popeffect_Junior2B>();
                if (pop != null)
                {
                    pop.enabled = false;
                    pop.enabled = true;
        ...
        else
        {
            ...
            if (_currentTopicData.Slides.Length > 0 && _currentSlideIndex < _currentTopicData.Slides.Length)
            {
                _currentTopicData.Slides[_currentSlideIndex].SetActive(false);
            }
    ...
    public void Next(bool value)
    {
        ...
        if (value && _next.TryGetComponent(out Popeffect_Junior2B pop))
        {
            pop.enabled = false;
            pop.enabled = true;
        }
    ...
    private void ShowCurrentSlideOnly()
    {
    ...
    private TopicData GetTopicDataForCurrentLesson(Topics expectedTopic)
    {
    ...
    {
        if (_currentTopicData == null) return;
