...
public class U2_R02_Junior2B : MonoBehaviour, Interfaces_Junior2B
{
    ...
    void OnDisable()
    {
        if (_tab1P == null || _tab2P == null) return;

        foreach (int index in _clickCheckIndex)
        {
            Transform btnTrans = null;
            int tab2ButtonCount = _tab1P.transform.childCount - 2;
            if (index < tab2ButtonCount)
            {
                if (index >= 0 && index < _tab1P.transform.childCount)
                {
                    btnTrans = _tab1P.transform.GetChild(index);
                }
            }
            else
            {
                int localIndex = index - tab2ButtonCount;
                if (localIndex >= 0 && localIndex < _tab2P.transform.childCount)
                {
                    btnTrans = _tab2P.transform.GetChild(localIndex);
                }
            }
