...
public class U10_L02_Junior2A : MonoBehaviour, Interfaces_Junior2A
{
    ...
    IEnumerator Starter()
    {
        foreach (Transform child in _tab.transform.GetChild(0)) if (child.GetComponent<Button>()) child.GetComponent<Button>().interactable = false;
    ...
    IEnumerator AutoRunAudios()
    {
        ...
        foreach (Transform child in _tab.transform.GetChild(0)) if (child.GetComponent<Button>()) child.GetComponent<Button>().interactable = true;
    ...
    public void PlayAudio(int index)
    {
        ...
        if (index % 2 == 0)
        {
            ...
            currentTabP.GetChild(1).GetComponent<PopEffect_Junior2A>().enabled = false;
            currentTabP.GetChild(1).GetComponent<PopEffect_Junior2A>().enabled = true;
        }
        else
        {
            currentTabP.GetChild(2).GetComponent<Image>().sprite = btnSprite;
            currentTabP.GetChild(2).GetComponent<PopEffect_Junior2A>().enabled = false;
            currentTabP.GetChild(2).GetComponent<PopEffect_Junior2A>().enabled = true;
        }
