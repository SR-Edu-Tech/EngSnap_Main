...
[Serializable]
public class U1_RP01_Junior2A_QuestionData
{
    public AudioClip SamQuestion, TinaAnswer;
    public string QuestionText;
    public string[] OptionText;
    public int CorrectAnsIndex;
}
public class U1_RP01_Junior2A : MonoBehaviour, Interfaces_Junior2A
{
    [SerializeField] U1_RP01_Junior2A_QuestionData[] _questionData;
    ...
    [SerializeField] Transform _tinaObj, _samObj;
    ...
    IEnumerator Starter()
    {
        ...
        _tinaObj.parent.gameObject.SetActive(true);
        _samObj.parent.gameObject.SetActive(true);
        _tinaObj.gameObject.SetActive(false);
        _samObj.gameObject.SetActive(false);
        ...
        _samObj.GetChild(0).GetComponent<TextMeshProUGUI>().text = _questionData[_currentQuestionIndex].QuestionText;
        _samObj.gameObject.SetActive(true);
        _audioSource.clip = _questionData[_currentQuestionIndex].SamQuestion;
        _audioSource.Play();
    ...
    IEnumerator Checker()
    {
        if (_questionData[_currentQuestionIndex].CorrectAnsIndex == _currentOptionIndex)
        {
            ...
            _tinaObj.GetChild(0).GetComponent<TextMeshProUGUI>().text = _questionData[_currentQuestionIndex].OptionText[_currentOptionIndex];
            _tinaObj.gameObject.SetActive(true);
            _audioSource.clip = _questionData[_currentQuestionIndex].TinaAnswer;
            _audioSource.Play();
            yield return new WaitForSeconds(_audioSource.clip.length);
            if (_currentQuestionIndex < _questionData.Length - 2)
            {
            ...
            {
                _tinaObj.parent.gameObject.SetActive(false);
                _samObj.parent.gameObject.SetActive(false);
        ...
        else
        {
            ...
            _optionObjs[_currentOptionIndex].GetComponent<WiggleEffect_Junior2A>().enabled = true;
            yield return new WaitForSeconds(_audioSource.clip.length);
            _optionObjs[_currentOptionIndex].GetComponent<Image>().color = Color.white;
        }
    }
