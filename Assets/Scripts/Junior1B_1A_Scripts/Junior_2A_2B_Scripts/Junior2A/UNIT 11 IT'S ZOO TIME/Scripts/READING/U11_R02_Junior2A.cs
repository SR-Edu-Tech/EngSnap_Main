...
public class U11_R02_Junior2A : MonoBehaviour, Interfaces_Junior2A
{
    ...
    // Bind your manual grid elements directly to this script passing explicit indexes (0, 1, 2...)
    public void PlayAudio(int index)
    {
        ...
        // Process completion validation logic
        ...
        // Toggle UI Graphics display elements
        if (currentTabP.childCount > 0 && currentTabP.GetChild(0).childCount > index)
        {
            Transform btnContainer = currentTabP.GetChild(0);
            Transform btn = btnContainer.GetChild(index);
                ...
                if (btnImg != null)
                {
                    ...
                    if (index % 2 == 0)
                    {
                        if (currentTabP.childCount > 1)
                        {
                            ...
                            if (currentTabP.GetChild(1).TryGetComponent(out PopEffect_Junior2A pop))
                            {
                                pop.enabled = false;
                                pop.enabled = true;
                            }
                    ...
                    else
                    {
                        if (currentTabP.childCount > 2)
                        {
                            ...
                            if (targetImg != null) targetImg.sprite = btnSprite;
                            if (currentTabP.GetChild(2).TryGetComponent(out PopEffect_Junior2A pop))
                            {
                                pop.enabled = false;
                                pop.enabled = true;
                            }
    ...
    public void Repeat()
    {
        // Re-triggers the manual item playback routine for the active selected sound clip
    ...
    private void ResetAllButtonColorsVisuals()
    {
        foreach (string clickId in _clickCheckIndex)
        {
            ...
            if (tabIndex >= 0 && tabIndex < TabPs.Length && TabPs[tabIndex] != null)
            {
                ...
                {
                    Transform btn = tabP.GetChild(0).GetChild(btnIndex);
                    if (btn.childCount > 1)
                    {
                        Image img = btn.GetChild(1).GetComponent<Image>();
                        if (img != null)
