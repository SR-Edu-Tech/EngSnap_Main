...
public class U7_A3_TeamSortController : MonoBehaviour
{
    [Header("Team Bins")]
    ...
    public bool overridePillSize = false;
    ...
    private void SpawnNextWord()
    {
        ...
        if (overridePillSize)
        {
            ...
            if (rt != null) rt.sizeDelta = pillCardSize;
        ...
        U7_TeamSortDragPill dragScript = activePillObj.GetComponent<U7_TeamSortDragPill>();
        if (dragScript == null) dragScript = activePillObj.AddComponent<U7_TeamSortDragPill>();
    ...
    private string HighlightTeamLetters(string word, string teamSpelling)
    {
        ...
        if (teamSpelling == "i_e" || teamSpelling == "o_e" || teamSpelling == "u_e")
        {
    ...
    public bool TryDropOnBin(GameObject targetObj, GameObject pillObj, Vector3 startPos)
    {
        ...
        while (current != null)
        {
            ...
            {
                U7_LongVowelTeamData team = currentLevel.teams[i];
                GameObject binObj = i < preplacedBins.Count && preplacedBins[i] != null
                    ? preplacedBins[i]
                    : (binsContainer != null && i < binsContainer.childCount ? binsContainer.GetChild(i).gameObject : null);
...
public class U7_TeamSortDragPill : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    private Vector3 startPosition;
    private Transform originalParent;
    private Canvas rootCanvas;
    private CanvasGroup canvasGroup;
    public U7_A3_TeamSortController controller;
