...
/// <summary>
/// Section C — Ending Blend Builder (Pages 46–47).
/// Builds words ending with a consonant blend.
/// Shows the default image in wordPictureImage when letters are not blended.
/// Upon completion, displays the Ending Blends Stage 3 Badge!
/// </summary>
public class U10_A3_EndingBuilderController : MonoBehaviour
{
    ...
    public TextMeshProUGUI resultWordText;    // Displays merged word (e.g. po<color=#D62828>nd</color>)
    ...
    public void SetupActivity(List<BlendWordData_Phonics_Junior> wordList)
    {
        ...
        if (titleText != null)
        {
            titleText.text = "Ending Blend Builder";
    ...
    private GameObject InstantiateTile(string chunk, bool isBlend, Transform parent)
    {
        ...
        else
        {
            ...
            bg.color  = isBlend ? new Color(1f, 0.9f, 0.9f, 1f) : new Color(0.96f, 0.96f, 0.96f, 1f);
            ...
            if (isBlend)
            {
                tmp.text = $"<color=#D62828><b>{chunk}</b></color>"; // Red for Ending Blend
            }
            else
            {
                tmp.text = $"<color=#000000><b>{chunk}</b></color>"; // Black for rest
            }
    ...
    private void OnBlendButtonClicked()
    {
        ...
        if (resultWordText != null)
        {
            ...
            resultWordText.text = $"<color=#000000><b>{pre}</b></color><color=#D62828><b>{data.targetBlend}</b></color>";
            resultWordText.fontSize = 72;
        }
    ...
    private IEnumerator AdvanceToNextWordRoutine()
    {
        ...
        else
        {
            ...
            // Show Ending Blends Stage 3 Badge 🏅!
            if (manager != null) manager.ShowStage3Reward();
        }
    }
    ...
    private List<string> BreakWordIntoChunks(string word, string blend)
    {
        ...
        int idx = !string.IsNullOrEmpty(lowerBlend) ? lowerWord.LastIndexOf(lowerBlend) : -1;

        if (idx > 0)
        {
            for (int i = 0; i < idx; i++) list.Add(word[i].ToString());
            list.Add(word.Substring(idx));
        }
        else
        {
