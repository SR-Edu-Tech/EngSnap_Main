...
[Serializable] // Fixed: Added so your array parameters show up correctly in the Unity Inspector!
...
public class U15_L01_Junior1A : MonoBehaviour
{
    ...
    private IEnumerator AutomatedLessonFlowRoutine()
    {
        ...
        if (_introAudio != null && _audioSource != null)
        {
            ...
            _audioSource.Play();
            // Fixed: Moved inside the null safety check block so it won't crash if unassigned
            yield return new WaitForSeconds(GetScaledAudioLength(_introAudio) + 0.5f);
        }

        // 2. Spawn buttons one-by-one
        for (int i = 0; i < totalItems; i++)
        {
        ...
        // 3. Play through the contractions sequentially
        for (int i = 0; i < totalItems; i++)
        {
        ...
        // 4. All done — unlock repeat button and show Next
    ...
    private IEnumerator RepeatAudio()
    {
        ...
        foreach (Transform button in _cardParent)
        {
            ContractionData data = _contractionRules[_currentAudioIndex];
