...
public class TextPopEffect_Junior1A : MonoBehaviour
{
    ...
    IEnumerator TextPop()
    {
        ...
        // Force layout update and wait exactly one frame for safety stability
        _tmpText.ForceMeshUpdate();
        yield return new WaitForEndOfFrame();

        float elapsed = 0f;
        
        // Scale the base settings by the speed multiplier
        float scaledStagger = stagger / speedMultiplier;
        float scaledFrequency = frequency * speedMultiplier;

        // Safely determine dynamic execution time limits
        ...
        while (elapsed < totalDuration)
        {
            ...
            // Force live regeneration on this frame so textInfo is always accurate
            _tmpText.ForceMeshUpdate();
            ...
            for (int i = 0; i < characterCount; i++)
            {
                ...
                // Bounds guard check to prevent cross-frame out of range exceptions
                ...
                // Transform the vertices directly based on the clean live layout frame data
                for (int v = 0; v < 4; v++)
                {
            ...
            // Push layout modifications directly to the working sub-mesh targets
        ...
        // Final restoration cleanup pass to guarantee perfectly straight text
        _tmpText.ForceMeshUpdate();
    }
}