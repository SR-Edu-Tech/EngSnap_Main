...
namespace Unit5.Q01.Junior1B
{
    [Serializable]
    public class U12_Q01_Junior1B_QuestionData
    {
    ...
    public class U12_Q01_Junior1B : MonoBehaviour, Interfaces_Junior1B
    {
        ...
        [SerializeField] U12_Q01_Junior1B_QuestionData[] _questions;
        ...
        private void SetupQuestionView()
        {
            ...
            // Populate text parameters first while elements are active and clean
            ...
            // Fire layout animations safely after UI positions are verified
            TriggerButtonPopEffects();
        }
        ...
        private void UpdateQuestionOptionsText()
        {
            ...
            for (int i = 0; i < physicalButtonsInScene; i++)
            {
                ...
                // Ensure base scale remains uncorrupted at absolute normal size
        ...
        private void ResetButtonsToDefaultState()
        {
            ...
            foreach (Transform obj in _buttonParent.transform)
            {
                ...
                // Explicitly preserve full scale before any component changes take place
                obj.localScale = Vector3.one;
                if (obj.TryGetComponent(out Popeffect_Junior1B pop)) pop.enabled = false;
            }
        }

        private void TriggerButtonPopEffects()
        {
            if (_buttonParent == null) return;
            ...
            foreach (Transform obj in _buttonParent.transform)
            {
                ...
                if (obj.TryGetComponent(out Popeffect_Junior1B pop))
                {
                    pop.enabled = false;
                    pop.enabled = true; // Safe to trigger now that scale calculation is synchronized
                }
            }
        }
        ...
        IEnumerator CheckAnswer(int index)
        {
            ...
            if (index < _buttonParent.transform.childCount)
            {
                ...
                if (index == currentQuestion.CorrectAnsIndex)
                {
                    ...
                    if (selectedButton.TryGetComponent(out Popeffect_Junior1B pop))
                    {
                        pop.enabled = false;
                        pop.enabled = true;
                    }
                ...
                else
                {
                    ...
                    if (selectedButton.TryGetComponent(out WiggleEffect_Junior1B wiggle))
                    {
            ...
            else
            {
                ...
                if (_audioButton != null) _audioButton.SetActive(false);
