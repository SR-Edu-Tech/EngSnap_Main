...
[Serializable]
public class U4_RP01_Junior1B_QuestionData
{
...
public class U4_RP01_Junior1B : MonoBehaviour, Interfaces_Junior1B
{
    [Header("=== Quiz Configuration ===")]
    [SerializeField] U4_RP01_Junior1B_QuestionData[] _questionData;
    ...
    void OnEnable()
    {
        // 💡 Gather the 3 buttons automatically when the object turns on
    ...
    private void GatherButtonsFromParent()
    {
        ...
        {
            Debug.LogError("<color=red>[Quiz Error]</color> You MUST drag your 'button parent' GameObject into the '_buttonParentContainer' slot in the inspector!");
            return;
        }

        // Loop through the children of 'button parent'
        for (int i = 0; i < _buttonParentContainer.childCount; i++)
        {
            ...
            // Dynamically register the click events via code so you don't have to map them manually
        ...
        Debug.Log($"<color=cyan>[Quiz System]</color> Successfully found and bound {_dynamicOptionButtons.Count} buttons inside button parent.");
    }
    ...
    IEnumerator Starter()
    {
        ...
        // 💡 Ensure all buttons are cleanly turned off during the intro sequence
    ...
    IEnumerator RevealOptionsRoutine()
    {
        ...
        {
            // First, make sure everything starts clean and white
            ...
            // 💡 FIX: Assign text strings FIRST while active, then display them to let UI layouts refresh
            for (int i = 0; i < layoutLimit; i++)
            {
                ...
                if (textComp != null)
                {
                    textComp.text = currentOptions[i];
                }
    ...
    IEnumerator Checker()
    {
        ...
        {
            // Hide incorrect choices immediately
            ...
            if (_dynamicOptionButtons[_currentOptionIndex] != null && _dynamicOptionButtons[_currentOptionIndex].TryGetComponent(out Popeffect_Junior1B pop))
            {
                pop.enabled = false;
                pop.enabled = true;
            }
        ...
        else
        {
            ...
            if (_dynamicOptionButtons[_currentOptionIndex] != null && _dynamicOptionButtons[_currentOptionIndex].TryGetComponent(out WiggleEffect_Junior1B wiggle))
            {
