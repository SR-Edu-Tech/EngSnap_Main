...
[Serializable]
public class U6_Q01_Junior2A_QuestionData
{
...
public class U6_Q01_Junior2A : MonoBehaviour, Interfaces_Junior2A
{
    [SerializeField] AudioSource _audioSource;
    [SerializeField] AudioClip _introClip, _wrongClip, _correctClip;
    [SerializeField] U6_Q01_Junior2A_QuestionData[] _questions;
    ...
    IEnumerator StartQuiz()
    {
        ...
        //_imageObj.SetActive(false); // Keeps the image hidden from the start
    ...
    IEnumerator CheckAnswer(int index)
    {
        if (index == _questions[_currentQuestionIndex].CorrectAnsIndex)
        {
            ...
            if (_buttonParent.transform.GetChild(index).TryGetComponent(out PopEffect_Junior2A pop))
            {
                pop.enabled = false;
                pop.enabled = true;
            }
        ...
        else
        {
            ...
            if (_buttonParent.transform.GetChild(index).TryGetComponent(out WiggleEffect_Junior2A1 wiggle))
            {
                wiggle.enabled = true;
            }
        ...
        if (_currentQuestionIndex < _questions.Length - 1)
        {
            ...
            // Removed the old 'if (_currentQuestionIndex == 3)' check that forced the image open.
        ...
        else
        {
            ...
            for (int i = 0; i < _questions.Length; i++)
            {
                ...
                if (i < _starWon)
                {
                    starChild.GetComponent<Image>().color = _wonStar;
                ...
                {
                    starChild.GetComponent<Image>().color = _loseStar;
                    if (starChild.TryGetComponent(out PopEffect_Junior2A pop)) pop.enabled = true;
                    starChild.gameObject.SetActive(true);
                }
            }
