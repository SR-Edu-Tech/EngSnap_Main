...
public class U1_SP01_Junior2A : MonoBehaviour, Interfaces_Junior2A
{
    ...
    IEnumerator AudioChecker(bool isMatch)
    {
        if (isMatch)
        {
            ...
            // Ensure skip button stays hidden on success
            if (_skipButtonObj != null) _skipButtonObj.SetActive(false);
        }
        else
        {
            ...
            yield return new WaitForSeconds(1.2f);
            _isProcessingResult = false;
            // Show skip button after a failed attempt
            if (_skipButtonObj != null) _skipButtonObj.SetActive(true);
    ...
    // Called by the Skip UI button when visible after a failed attempt
    public void OnSkipButtonPressed()
    {
        // Advance to next clip if possible
        if (_currentAudioIndex < _questionClips.Length - 1)
        {
            _currentAudioIndex++;
            ShowTargetWord();
        }
        // Hide skip button again
        if (_skipButtonObj != null) _skipButtonObj.SetActive(false);
    }
