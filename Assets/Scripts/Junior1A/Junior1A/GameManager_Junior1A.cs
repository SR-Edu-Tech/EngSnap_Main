...
public class GameManager_Junior1A : MonoBehaviour
{
    ...
    public void SelectLesson(int lessonIndex)
    {
        ...
        if (_selectedLessonIndex >= 0 && _selectedLessonIndex < _lessons.Length)
        {
            ...
            foreach (TopicData topic in _lessons[_selectedLessonIndex].Topics)
            {
                ...
                {
                    _topicParent.transform.GetChild(0).GetChild(0).GetChild(0).GetChild(_currentTopicIndex).GetChild(1).GetComponent<Image>().enabled = true;
                    Debug.Log($"Completed Topic: {topic.TopicType}");
                }
                else _topicParent.transform.GetChild(0).GetChild(0).GetChild(0).GetChild(_currentTopicIndex).GetChild(1).GetComponent<Image>().enabled = false;
                _currentTopicIndex++;
            }
        }
    }
    ...
    public void Next(bool value) => _next.SetActive(value);
    ...
    void CompleteCurrentTopic()
    {
        ...
        foreach (TopicData topic in _lessons[_selectedLessonIndex].Topics)
        {
            if (topic.IsCompleted) _topicParent.transform.GetChild(0).GetChild(0).GetChild(0).GetChild(_currentTopicIndex).GetChild(1).GetComponent<Image>().enabled = true;
            else
            {
                _topicParent.transform.GetChild(0).GetChild(0).GetChild(0).GetChild(_currentTopicIndex).GetChild(1).GetComponent<Image>().enabled = false;
                _allDone = false;
            }
            _currentTopicIndex++;
        }
