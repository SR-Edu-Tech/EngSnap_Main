...
public class Masters_WordSearchManager : MonoBehaviour
{
    [Header("Grid")]
    ...
    public Masters_WordSearchCell cellPrefab;
    ...
    public Masters_WordItemUI wordItemPrefab;
    ...
    private Masters_WordSearchCell[,] grid;
    private List<Masters_WordSearchCell> selection  = new List<Masters_WordSearchCell>();
    private Vector2Int selDir;
    private bool       isDragging;

    private Dictionary<string, Masters_WordItemUI> wordUIMap  = new Dictionary<string, Masters_WordItemUI>();
    private HashSet<string>                foundWords = new HashSet<string>();
    private Image dragPill;

    private HashSet<Masters_WordSearchCell> scaledCells = new HashSet<Masters_WordSearchCell>();
    ...
    // Load new topic
    public void LoadTopic(Masters_WordTopicData topic)
    {
    ...
    void Update()
    {
        ...
        Masters_WordSearchCell hit = GetCellAtScreenPos(screenPos);
    ...
    void CreateWordListUI()
    {
        ...
        {
            Masters_WordItemUI item = Instantiate(wordItemPrefab, wordListParent);
            item.Init(word);
            wordUIMap[word] = item;
        }
    }
    ...
    void BuildGrid()
    {
        ...
        for (int i = gridParent.childCount - 1; i >= 0; i--)
        {
            ...
            if (t.GetComponent<Masters_WordSearchCell>() != null) Destroy(t.gameObject);
        }
        ...
        grid = new Masters_WordSearchCell[rows, cols];
        for (int r = 0; r < rows; r++)
            for (int c2 = 0; c2 < cols; c2++)
            {
                Masters_WordSearchCell cell = Instantiate(cellPrefab, gridParent);
    ...
    // Find word cells
    List<Masters_WordSearchCell> FindWordCells(string word)
    {
        ...
        for (int r = 0; r < rows; r++)
            for (int c = 0; c < cols; c++)
                ...
                {
                    List<Masters_WordSearchCell> cells = new List<Masters_WordSearchCell>();
                    bool ok = true;
                    for (int i = 0; i < word.Length; i++)
                    {
    ...
    public void StartSelection(Masters_WordSearchCell cell)
    {
    ...
    public void ContinueSelection(Masters_WordSearchCell cell)
    {
        ...
        {
            Masters_WordSearchCell removed = selection[selection.Count - 1];
        ...
        Masters_WordSearchCell first = selection[0];
        Masters_WordSearchCell last  = selection[selection.Count - 1];
    ...
    void SetCellDragScale(Masters_WordSearchCell cell, bool enlarged)
    {
    ...
    Masters_WordSearchCell GetCellAtScreenPos(Vector2 screenPos)
    {
        if (grid == null) return null;

        float bestDist = float.MaxValue;
        Masters_WordSearchCell bestCell = null;
        ...
        for (int r = rMin; r <= rMax; r++)
            ...
            {
                Masters_WordSearchCell cell = grid[r, c];
    ...
    Vector2 GetCellScreenCenter(Masters_WordSearchCell cell)
    {
    ...
    float GetCellScreenSize(Masters_WordSearchCell cell)
    {
        Vector3[] corners = new Vector3[4];
        cell.GetComponent<RectTransform>().GetWorldCorners(corners);
    ...
    Image SpawnPill(List<Masters_WordSearchCell> cells, Color color)
    {
        if (cells == null || cells.Count == 0 || pillPrefab == null || pillParent == null)
            return null;

        Masters_WordSearchCell first = cells[0];
        Masters_WordSearchCell last  = cells[cells.Count - 1];
    ...
    Vector3 GetCellWorldCenter(Masters_WordSearchCell cell)
    {
    ...
    float GetCellWorldSize(Masters_WordSearchCell cell)
    {
        Vector3[] c = new Vector3[4];
        cell.GetComponent<RectTransform>().GetWorldCorners(c);
