...
public class Masters_YeahAndNah_Reading_LessonOne : Masters_Lesson {
    ...
    [SerializeField]
    private Button loveContextButton;
    [SerializeField]
    private TextMeshProUGUI loveContextButtonTMP;
    [SerializeField]
    private Button likeContextButton;
    [SerializeField]
    private TextMeshProUGUI likeContextButtonTMP;
    [SerializeField]
    private Button hateContextButton;
    [SerializeField]
    private TextMeshProUGUI hateContextButtonTMP;
    [SerializeField]
    ...
    protected override void Awake() {
        ...
        loveContextButton.onClick.AddListener(() => {
            OnContextButtonClicked(PhraseContext.Love);
        });
        likeContextButton.onClick.AddListener(() => {
            OnContextButtonClicked(PhraseContext.Like);
        });
        hateContextButton.onClick.AddListener(() => {
            OnContextButtonClicked(PhraseContext.Hate);
        });
    ...
    private void OnContextButtonClicked(PhraseContext phraseContext) {
        ...
        switch (phraseContext) {
            case PhraseContext.Love:
                // Correct Answer
                if (currentPhraseAndContext.context == phraseContext) {
                    Masters_AudioManager.Instance.PlaySoundEffect(Masters_SFX.Correct);
                    loveContextButtonTMP.color = correctButtonColor;
                    numberOfCorrectAnswers++;
                } else {
                    Masters_AudioManager.Instance.PlaySoundEffect(Masters_SFX.Incorrect);
                    loveContextButtonTMP.color = wrongButtonColor;
                }

                canSelectOption = false;
                Invoke(GO_TO_NEXT_QUESTION, timeToNextQuestion);
                break;
            case PhraseContext.Like:
                // Correct Answer
                if (currentPhraseAndContext.context == phraseContext) {
                    Masters_AudioManager.Instance.PlaySoundEffect(Masters_SFX.Correct);
                    likeContextButtonTMP.color = correctButtonColor;
                    numberOfCorrectAnswers++;
                } else {
                    Masters_AudioManager.Instance.PlaySoundEffect(Masters_SFX.Incorrect);
                    likeContextButtonTMP.color = wrongButtonColor;
                }

                canSelectOption = false;
                Invoke(GO_TO_NEXT_QUESTION, timeToNextQuestion);
                break;
            case PhraseContext.Hate:
                // Correct Answer
                if (currentPhraseAndContext.context == phraseContext) {
                    Masters_AudioManager.Instance.PlaySoundEffect(Masters_SFX.Correct);
                    hateContextButtonTMP.color = correctButtonColor;
                    numberOfCorrectAnswers++;
                } else {
                    Masters_AudioManager.Instance.PlaySoundEffect(Masters_SFX.Incorrect);
                    hateContextButtonTMP.color = wrongButtonColor;
                }
    ...
    private void GoToNextQuestion() {
        ...
        loveContextButtonTMP.color = defaultButtonColor;
        likeContextButtonTMP.color = defaultButtonColor;
        hateContextButtonTMP.color = defaultButtonColor;

        SetMiniQuizQuestion();
    }

    private IEnumerator ContextButtonAnimations() {
        RectTransform casualContextRectTransform = loveContextButton.GetComponent<RectTransform>();
        RectTransform formalContextRectTransform = likeContextButton.GetComponent<RectTransform>();
        RectTransform friendlyContextRectTransform = hateContextButton.GetComponent<RectTransform>();
