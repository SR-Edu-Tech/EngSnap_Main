...
public class Masters_SelfIntroduction_Listening_LessonOne : Masters_Lesson {
    ...
    private IEnumerator PhraseCardsPopUpAnimation() {
        ...
        for (int i = 0; i < phraseCardArray.Length; i++) {
            ...
            Vector3 viewportPos = Camera.main.WorldToViewportPoint(phraseCardArray[i].button.transform.position);
    ...
    private void OnPhraseCardButtonClicked(PhraseCard phraseCard) {
        ...
        if (!phraseCardHashSet.Contains(phraseCard)) {
            ...
            expressionCountTMP.text = $"{phraseCardHashSet.Count}/13";

            if (phraseCardHashSet.Count == 13) {
