...
/// <summary>
/// Core Game Manager for Change Voice And Sound Smart - Roleplay Lesson 2.
/// Simulates a scene-based roleplay where the player selects a scene and forms a reply
/// from a bank of word buttons (Word Sorting mechanic) in response to an NPC.
/// </summary>
public class Masters_ChangeVoiceAndSoundSmart_Roleplay_LessonTwo : Masters_Lesson {
    ...
    public class RoleplayTurn {
        ...
        [Tooltip("The jumbled words presented to the player to form their sentence.")]
    ...
    private void OnArrangeWordButtonClicked(Masters_ArrangeWordButton arrangeWordButton) {
        ...
        if (arrangeWordButton.GetIsInBox() == false) {
            ...
            if (slateWordsParentTransform.childCount > 0 && canClickCheck && checkButton != null) {
    ...
    /// <summary>
    /// Reads the player's constructed sentence, normalizes it (removes spaces/punctuation/casing),
    /// and checks it against the valid sentence array.
    /// </summary>
