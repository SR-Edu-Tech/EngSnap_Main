...
public class Masters_LetsChoose_Reading_LessonThree : Masters_Lesson {
    ...
    private void PlayDialogueLineByLine(DialogueSet dialogueSet) {
        if (!dialogueSetHashSet.Contains(dialogueSet)) {
            // New dialogue interacted
            dialogueSetHashSet.Add(dialogueSet);
            if (dialogueCountTMP != null) {
                dialogueCountTMP.text = $"{dialogueSetHashSet.Count}/{dialogueSetArray.Length}";
            }
            if (dialogueSetHashSet.Count == dialogueSetArray.Length) {
                if (startQuizButton != null) {
                    startQuizButton.interactable = true;
                    // Try getting the animator in case it's on the button prefab
                    Masters_StartMiniQuizButtonAnimator anim = startQuizButton.GetComponent<Masters_StartMiniQuizButtonAnimator>();
                    if (anim != null) {
                        anim.StartMiniQuizButtonAnimation();
                    } else {
                        startQuizButton.transform.DOPunchScale(Vector3.one * 0.1f, 0.5f);
                    }
                } else {
                    // Fallback if no start button: move to quiz automatically
                    Invoke(nameof(StartQuizPhase), 2f);
                }
            }
        }
        ...
        for (int i = 0; i < dialogueSetArray.Length; i++) {
            if (dialogueSet == dialogueSetArray[i]) {
                if (dialogueSet.gameObject != null) dialogueSet.gameObject.SetActive(true);
            ...
            if (dialogueSetArray[i].gameObject != null) dialogueSetArray[i].gameObject.SetActive(false);
        }
    }
