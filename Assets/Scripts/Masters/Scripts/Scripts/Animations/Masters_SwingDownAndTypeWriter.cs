...
public class Masters_SwingDownAndTypeWriter : MonoBehaviour {
    ...
    private void OnEnable() {
        SwingDownAndTypeWriterAnimation();
    ...
    private void SwingDownAndTypeWriterAnimation() {
        Vector2 targetPosition = rectTransform.anchoredPosition;
        rectTransform.anchoredPosition = new Vector2(targetPosition.x, 150);

        rectTransform.DOAnchorPos(targetPosition, animationSpeed).SetEase(Ease.OutExpo).OnComplete(() => {
