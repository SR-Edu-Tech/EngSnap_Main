...
/// <summary>
/// Core Game Manager for Smart Alternatives - Speaking Lesson One.
/// </summary>
public class Masters_SmartAlternatives_Speaking_LessonOne : Masters_Lesson {
    ...
    [SerializeField] private float timeToLoadNextQuestion = 1.8f;
    ...
    private void OnHintButtonClicked() {
        // Hint system removed - answer is shown active from start.
    }
    ...
    private IEnumerator StartingAnimationCoroutine() {
        ...
        if (sliderRectTransform) sliderRectTransform.anchoredPosition = sliderStartPosition;
        if (micRectTransform) micRectTransform.anchoredPosition = micStartPosition;
        ...
        if (debugRectTransform) debugRectTransform.anchoredPosition = debugStartPosition;
        ...
        if (sliderRectTransform) sliderRectTransform.DOAnchorPos(sliderTargetPosition, animationSpeed).SetEase(Ease.OutExpo);
        ...
        yield return new WaitForSeconds(timeBetweenEachAnimation);
        if (micRectTransform) micRectTransform.DOAnchorPos(micTargetPosition, animationSpeed).SetEase(Ease.OutExpo);
        if (debugRectTransform) debugRectTransform.DOAnchorPos(debugTargetPosition, animationSpeed).SetEase(Ease.OutExpo);
    }
    ...
    private void SetQuestionState() {
        ...
        // Show answer active immediately from start
        ...
        if (currentQuestion.expectedAnswerAudioClip != null) {
            Masters_AudioManager.Instance.PlayVoiceOver(currentQuestion.expectedAnswerAudioClip);
        }
    }
