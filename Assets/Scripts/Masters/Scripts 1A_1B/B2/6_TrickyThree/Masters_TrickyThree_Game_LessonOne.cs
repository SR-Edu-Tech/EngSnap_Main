...
public class Masters_TrickyThree_Game_LessonOne : Masters_Lesson {
    ...
    protected virtual void SpawnChipsForQuestion(GameQuestion q) {
        ClearActiveChips();

        List<string> options = new List<string>();
        if (!string.IsNullOrEmpty(q.correctAnswer)) options.Add(q.correctAnswer);
        if (!string.IsNullOrEmpty(q.distractorOne)) options.Add(q.distractorOne);
        if (!string.IsNullOrEmpty(q.distractorTwo)) options.Add(q.distractorTwo);
