...
public class Masters_AbbreviationsAndAcronyms_Writing_LessonOne : Masters_Lesson {
    ...
    private void OnRetryButtonClicked() {
        completedRectTransform.DOScale(Vector3.zero, animationSpeed).SetEase(Ease.OutExpo).OnComplete(() => {
            ...
            correctlyFilled = 0;
            correctlyFilledTMP.text = $"Correctly Filled: {correctlyFilled}/6";
        });
    }
    ...
    private void LoadNextSetOfStatementsAndButtons() {
        ...
        if(setIndex >= statementGameObjectArray.Length) {
