...
public class Masters_Collocations_Reading_LessonTwo : Masters_Lesson {
    ...
    public class WordWebData {
        public Masters_Collocations_Listening_LessonOne.CollocationHub hubId;
    ...
    private void EnsureDefaultWebsData() {
        if (webs == null || webs.Length < 4 || webs[0] == null || webs[0].correctPartners == null || webs[0].correctPartners.Length == 0) {
            webs = new WordWebData[] {
                new WordWebData {
                    hubId = Masters_Collocations_Listening_LessonOne.CollocationHub.GET,
                    hubDisplayName = "get",
                    correctPartners = new string[] { "ready", "along with", "a job", "angry", "dressed", "permission" }
                },
                new WordWebData {
                    hubId = Masters_Collocations_Listening_LessonOne.CollocationHub.CATCH,
                    hubDisplayName = "catch",
                    correctPartners = new string[] { "a bus", "a cold", "a train", "a thief", "someone's eye", "fire" }
                },
                new WordWebData {
                    hubId = Masters_Collocations_Listening_LessonOne.CollocationHub.SAVE,
                    hubDisplayName = "save",
                    correctPartners = new string[] { "water", "time", "a life", "electricity", "money", "a seat" }
                },
                new WordWebData {
                    hubId = Masters_Collocations_Listening_LessonOne.CollocationHub.IDEA,
                    hubDisplayName = "idea",
    ...
    private void SpawnTilesForCurrentWeb(WordWebData activeWeb) {
        ...
        List<(string text, Masters_Collocations_Listening_LessonOne.CollocationHub hub)> tileList = new List<(string text, Masters_Collocations_Listening_LessonOne.CollocationHub hub)>();
        ...
        for (int i = 0; i < countCorrect; i++) {
            ...
            if (activeWeb.hubId == Masters_Collocations_Listening_LessonOne.CollocationHub.IDEA) {
        ...
        for (int w = 0; w < webs.Length; w++) {
            if (webs[w].hubId != activeWeb.hubId && webs[w].correctPartners != null && webs[w].correctPartners.Length > 0) {
                ...
                if (webs[w].hubId == Masters_Collocations_Listening_LessonOne.CollocationHub.IDEA) decoyText = decoyText + " idea";
