...
public class Masters_Collocations_Game_LessonTwo : Masters_Lesson {
    ...
    private void DeactivateObsoleteBaseUI() {
        ...
        Transform topBar = transform.Find("TopBarContainer");
        if (topBar != null) topBar.gameObject.SetActive(false);

        Transform score = transform.Find("ScoreTMP") ?? transform.Find("Score");
        if (score != null) score.gameObject.SetActive(false);

        Transform progCount = transform.Find("ProgressionCountTMP");
        if (progCount != null) progCount.gameObject.SetActive(false);

        Transform memGrid = transform.Find("MemoryGridContainer");
        if (memGrid != null) memGrid.gameObject.SetActive(false);
        ...
        foreach (var tmp in allTMPs) {
            ...
            if (txt.Contains("SCORE") || txt.Contains("Festival") || txt.Contains("FLIPS") || txt.Contains("PAIRS: 0/8") || gName.Contains("score")) {
    ...
    private void GenerateFloorTiles() {
        ...
        // Clear existing tiles in floorContainer to prevent double tiles and overlapping text
        for (int i = floorContainer.childCount - 1; i >= 0; i--) {
            Transform child = floorContainer.GetChild(i);
            if (child != null) {
        ...
        for (int i = 0; i < spawnList.Count; i++) {
            ...
            G02TileUI tileUI = tileObj.GetComponent<G02TileUI>();
    ...
    private void UpdateTitleAndUIComponents() {
        ...
        foreach (var tmp in allTMPs) {
            ...
            if (lowerName.Contains("lessontitle") || lowerName.Contains("title") || textVal.Contains("Festival") || textVal.Contains("Twin Match") || textVal.Contains("Polished") || textVal.Contains("Greeting Memory")) {
                tmp.gameObject.SetActive(true);
                tmp.text = "G02 Two Halves to Make One";
                tmp.color = Color.white;
            }
            if (lowerName.Contains("heading") || lowerName.Contains("header") || textVal.Contains("GROOVE") || textVal.Contains("GAME BRANCH")) {
                tmp.gameObject.SetActive(true);
                tmp.text = "GAME BRANCH (Game Bench)";
                tmp.color = new Color(0.13f, 0.77f, 0.36f);
            }
        }
        ...
        if (g02TitleTMP != null) {
            ...
            g02TitleTMP.color = new Color(1f, 0.85f, 0.05f, 1f); // Yellow
        }
