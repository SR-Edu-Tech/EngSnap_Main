...
/// <summary>
/// Core Listening 2 controller for Unit 1: Polished Communication (Book 2A Reference Base).
/// Uses Masters_UniversalSortPhraseCard and Masters_UniversalSortBin.
/// Card spawns, student taps on the correct bin (FORMAL vs INFORMAL) to sort it.
/// </summary>
public class Masters_PolishedCommunication_Listening_LessonTwo : Masters_Lesson {

    [System.Serializable]
    public class SortTileData {
        public string expressionText;
        public AudioClip expressionAudio;
        public Masters_Unit1_FallingSortCategory category;
    }

    [Header("Listening L2 Data")]
    [SerializeField] protected SortTileData[] sortTiles;
    [SerializeField] protected Masters_UniversalSortBin[] sortBinArray;
    [SerializeField] protected Masters_UniversalSortPhraseCard sortPhraseCard;
    [SerializeField] protected RectTransform sortPhraseRestPointRectTransform;
    [SerializeField] protected TextMeshProUGUI progressTMP;
    [SerializeField] protected float animationSpeed = 0.4f;
    [SerializeField] protected int passThreshold = 6;

    protected int currentTileIndex = 0;
    protected int correctSorts = 0;
    ...
    protected virtual void ConfigureSortBins() {
        if (sortBinArray != null) {
            for (int i = 0; i < sortBinArray.Length; i++) {
                if (sortBinArray[i] != null) {
                    if (i == 0) {
                        sortBinArray[i].gameObject.SetActive(true);
                        sortBinArray[i].SetSortId(0); // FORMAL = 0
                        SetBinLabelText(sortBinArray[i], "FORMAL");
                    } else if (i == 1) {
                        sortBinArray[i].gameObject.SetActive(true);
                        sortBinArray[i].SetSortId(1); // INFORMAL = 1
                        SetBinLabelText(sortBinArray[i], "INFORMAL");
                    } else {
                        // Disable extra bins carried over from Book 2B (bins 3 and 4)
    ...
    protected virtual void SetBinLabelText(Masters_UniversalSortBin bin, string text) {
        if (bin == null) return;
    ...
    private void LoadTile(int index) {
        ...
        if (sortPhraseCard != null) {
            int sortId = (int)tile.category; // FORMAL = 0, INFORMAL = 1
    ...
    protected virtual void OnSortBinClicked(Masters_UniversalSortBin sortBin) {
        ...
        int expectedSortId = (int)tile.category; // FORMAL = 0, INFORMAL = 1
    ...
    protected virtual IEnumerator NextTileRoutine() {
