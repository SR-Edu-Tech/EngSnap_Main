...
public class Masters_PolishedCommunication_Quiz_LessonOne : Masters_Lesson {
    ...
    [Header("Repeat Audio Button")]
    [SerializeField] protected Button repeatAudioButton;
    ...
    private void CleanupDuplicateRepeatButtons() {
        Transform[] siblings = (transform.parent != null) ? transform.parent.GetComponentsInChildren<Transform>(true) : GetComponentsInChildren<Transform>(true);
        List<GameObject> toDestroy = new List<GameObject>();
        int keptCount = 0;
        foreach (var t in siblings) {
            if (t != null && t.name.Equals("RepeatAudioButton")) {
                keptCount++;
                if (keptCount > 1) {
    ...
    protected virtual void EnsureRepeatAudioButton() {
        if (!Application.isPlaying) return;
        CleanupDuplicateRepeatButtons();

        Transform btnTrans = transform.Find("RepeatAudioButton");
        if (btnTrans == null && transform.parent != null) {
            btnTrans = transform.parent.Find("RepeatAudioButton");
        }

        GameObject btnObj;
        if (btnTrans == null) {
            btnObj = new GameObject("RepeatAudioButton");
            ...
            img.color = new Color(0.12f, 0.40f, 0.85f, 1f);
        } else {
        ...
        repeatAudioButton = btnComp;

        Transform textTrans = btnObj.transform.Find("SpeakerIcon");
        GameObject textObj = (textTrans != null) ? textTrans.gameObject : new GameObject("SpeakerIcon");
        ...
        TextMeshProUGUI iconTMP = textObj.GetComponent<TextMeshProUGUI>();
        if (iconTMP == null) iconTMP = textObj.AddComponent<TextMeshProUGUI>();
        iconTMP.enabled = true;
        iconTMP.text = "REPEAT";
        ...
        if (repeatAudioButton != null) {
            repeatAudioButton.transform.DOKill(true);
            repeatAudioButton.transform.localScale = new Vector3(3f, 3f, 1f);
            repeatAudioButton.gameObject.SetActive(true);
            repeatAudioButton.onClick.RemoveAllListeners();
            repeatAudioButton.onClick.AddListener(OnRepeatAudioButtonClicked);
        }
    }

    protected virtual void OnRepeatAudioButtonClicked() {
        if (repeatAudioButton != null) {
            repeatAudioButton.transform.DOKill(true);
            repeatAudioButton.transform.localScale = new Vector3(3f, 3f, 1f);
        }
    ...
    protected virtual void ReplayCurrentQuestionAudio() {
        if (currentQuiz != null && currentQuiz.questionAudioClip != null && Masters_AudioManager.Instance != null) {
            Masters_AudioManager.Instance.StopVoiceOver();
            Masters_AudioManager.Instance.PlayVoiceOver(currentQuiz.questionAudioClip);
        }
    }
