...
/// <summary>
/// Core Game Manager for Unit 3A: Boost Someone Up - Roleplay Lesson Two (RP02: Free Scene - Lift a Friend Up).
/// Standalone Book 3A base controller that simulates a scene-based roleplay where the player selects a scene and forms two sequential tone-contrasting turns from a bank of phrase buttons.
/// </summary>
public class Masters_BoostSomeoneUp_Roleplay_LessonTwo : Masters_Lesson {
    ...
    public class SceneData {
        ...
        public AudioClip ariaSetupAudioClip;
        [TextArea] public string ariaSetupText;
    ...
    [SerializeField] protected TextMeshProUGUI ariaSetupTMP;
    ...
    protected virtual void StartScene(SceneData sceneData) {
        ...
        if (ariaSetupTMP != null) {
            if (!string.IsNullOrEmpty(activeScene.ariaSetupText)) {
                ariaSetupTMP.gameObject.SetActive(true);
                ariaSetupTMP.text = activeScene.ariaSetupText;
            } else {
                ariaSetupTMP.gameObject.SetActive(false);
            }
        }
        ...
        if (activeScene.ariaSetupAudioClip != null && Masters_AudioManager.Instance != null) {
            Masters_AudioManager.Instance.PlayVoiceOver(activeScene.ariaSetupAudioClip);
            Invoke(nameof(StartNextTurn), activeScene.ariaSetupAudioClip.length + 0.2f);
        } else {
            StartNextTurn();
        }
    }
    ...
    protected virtual void StartNextTurn() {
        ...
        if (turn.turnType == TurnType.NPCTurn) {
            ...
            if (turn.npcAudioClip != null && Masters_AudioManager.Instance != null) {
                ...
                delay = turn.npcAudioClip.length + 0.2f;
            }
