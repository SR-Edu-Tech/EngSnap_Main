...
{
    public class U6_SA_AudioManager_Masters_Phonics : MonoBehaviour
    {
        public static U6_SA_AudioManager_Masters_Phonics Instance { get; private set; }
        ...
        [Header("Unit 6 Specific SFX")]
        [Tooltip("U06_SFX_wand: Soft magical chime for the e tile landing")]
        public AudioClip wandSparkleClip;
        [Tooltip("U06_SFX_vowel_stretch: Rising pitch swell for vowel going long")]
        public AudioClip vowelStretchClip;
        [Tooltip("U06_SFX_soften: Edge-rounding whoosh for c or g going soft")]
        public AudioClip softenClip;
        [Tooltip("U06_SFX_picture_morph: Quick shimmer when picture transforms")]
        public AudioClip pictureMorphClip;
        ...
        public static AudioClip ResolveAudio(string name)
        {
            ...
            if (audioClipCache == null)
            {
                ...
                foreach (var c in allClips)
                {
                    ...
                    string clean = cName.Replace("u06_vo_", "").Replace("u06_wrd_", "").Replace("u06_sfx_", "")
                                        .Replace("u05_wrd_", "").Replace("u04_wrd_", "").Replace("chunk_", "").Trim();
            ...
            if (audioClipCache.TryGetValue($"u06_wrd_{target}", out AudioClip wrdClip)) return wrdClip;
            if (audioClipCache.TryGetValue($"u06_vo_{target}", out AudioClip voClip)) return voClip;
            if (audioClipCache.TryGetValue($"u06_sfx_{target}", out AudioClip sfxClip)) return sfxClip;
        ...
        public void PlaySFX(string sfxName)
        {
            ...
            switch (sfxName.ToLower())
            {
                ...
                case "wand":
                case "sparkle":
                    clip = wandSparkleClip;
                    break;
                case "vowel_stretch":
                case "stretch":
                    clip = vowelStretchClip;
                    break;
                case "soften":
                case "glide":
                    clip = softenClip;
