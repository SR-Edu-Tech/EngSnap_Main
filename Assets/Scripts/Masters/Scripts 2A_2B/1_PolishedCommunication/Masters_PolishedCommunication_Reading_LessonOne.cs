...
/// <summary>
/// Core Reading 1 controller for Unit 1: Polished Communication (Book 2A Reference Base).
/// R01 Find the Partner (Formal <-> Informal): Displays an expression at the top notice board and 4 chip buttons below.
/// Student taps the correct partner expression from the chips.
/// </summary>
public class Masters_PolishedCommunication_Reading_LessonOne : Masters_Lesson {
    ...
    private void EnsureLessonTitleAndExpressionBox() {
        ...
        foreach (var tmp in allTMPs) {
            ...
            if (tmp.name.ToLower().Contains("lessontitle") || tmp.name.ToLower().Contains("title") || tmp.text.Contains("Find the Partner") || tmp.text.Contains("R01")) {
                tmp.gameObject.SetActive(true);
                if (tmp.text.Contains("Hear") || tmp.text.Contains("Listen") || string.IsNullOrWhiteSpace(tmp.text)) {
                    tmp.text = "R01 Find the Partner (Formal \u2194 Informal)";
                }
                continue;
            }
    ...
    public void InitializeRoundsIfEmpty() {
        if (rounds != null && rounds.Length > 0) return;

        rounds = new PartnerRoundData[] {
            new PartnerRoundData { shownExpression = "Hi! / Hello!  (FORMAL)", correctPartner = "Hey!", wrongDistractors = new string[] { "What's up?", "Howdy?", "Chill" } },
            new PartnerRoundData { shownExpression = "How are you?  (FORMAL)", correctPartner = "What's up?", wrongDistractors = new string[] { "Hey!", "Buddy", "Shades" } },
            new PartnerRoundData { shownExpression = "How is it going with you?  (FORMAL)", correctPartner = "Howdy?", wrongDistractors = new string[] { "Chill", "Goof up", "Get it" } },
            new PartnerRoundData { shownExpression = "Take it easy!  (FORMAL)", correctPartner = "Chill", wrongDistractors = new string[] { "Shades", "Buddy", "Hey!" } },
            new PartnerRoundData { shownExpression = "Friend  (FORMAL)", correctPartner = "Buddy", wrongDistractors = new string[] { "Shades", "Chill", "What's up?" } },
            new PartnerRoundData { shownExpression = "To understand  (FORMAL)", correctPartner = "Get it", wrongDistractors = new string[] { "Goof up", "Chill", "Buddy" } },
            new PartnerRoundData { shownExpression = "Made a mistake  (FORMAL)", correctPartner = "Goof up", wrongDistractors = new string[] { "Get it", "Shades", "Chill" } },
            new PartnerRoundData { shownExpression = "Sunglasses  (FORMAL)", correctPartner = "Shades", wrongDistractors = new string[] { "Buddy", "Hey!", "Howdy?" } },
            new PartnerRoundData { shownExpression = "Shades  (INFORMAL)", correctPartner = "Sunglasses", wrongDistractors = new string[] { "Friend", "Hello!", "To understand" } },
            new PartnerRoundData { shownExpression = "Buddy  (INFORMAL)", correctPartner = "Friend", wrongDistractors = new string[] { "Sunglasses", "Hello!", "Made a mistake" } },
            new PartnerRoundData { shownExpression = "What's up?  (INFORMAL)", correctPartner = "How are you?", wrongDistractors = new string[] { "To understand", "Friend", "Take it easy!" } },
            new PartnerRoundData { shownExpression = "Goof up  (INFORMAL)", correctPartner = "Made a mistake", wrongDistractors = new string[] { "How are you?", "To understand", "Hello!" } }
        };
    }
