...
/// <summary>
/// Core controller for Unit 1: Polished Communication - Game Lesson One (G01: Street Dash / Falling Sort).
/// Standalone implementation independent of older book scripts.
/// Manages falling card spawning, two-bin drag-drop logic, scoring, timer, and progression.
/// </summary>
public class Masters_PolishedCommunication_Game_LessonOne : Masters_Lesson {

    [System.Serializable]
    public class SortPuzzle {
        public string expression;
        public Masters_Unit8_FallingSortCategory sortType;
        public Masters_Unit2_FallingSortCategory unit2SortType;
        public Masters_Unit3_FallingSortCategory unit3SortType;
        public Masters_Unit4_FallingSortCategory unit4SortType;
        public AudioClip audioClip;
    }

    [Header("Game Data")]
    [SerializeField]
    protected SortPuzzle[] sortPuzzleArray;
    [SerializeField]
    protected Masters_FallingSortBin[] sortBinArray;
    [SerializeField]
    protected Masters_FallingSortPhraseCard phraseCardPrefab;

    [Header("Game UI")]
    [SerializeField]
    protected RectTransform topSpawnPoint;
    [SerializeField]
    ...
    [SerializeField]
    protected float gameDuration = 60f;
    [SerializeField]
    protected float initialSpawnDelay = 3f;
    [SerializeField]
    protected float initialSpawnInterval = 4f;
    [SerializeField]
    protected float minSpawnInterval = 1.0f;
    [Tooltip("How much to reduce the spawn interval every second.")]
    [SerializeField]
    protected float spawnIntervalDecreaseRate = 0.05f;

    [Header("Falling Settings")]
    [SerializeField]
    protected float initialFallSpeed = 200f; // Pixels per second
    [Tooltip("How much the falling speed increases every second.")]
    [SerializeField]
    protected float fallSpeedIncreaseRate = 5f;
    [SerializeField]
    protected float maxFallSpeed = 600f;
    [SerializeField]
    protected float snapAnimationSpeed = 0.2f;
    ...
    protected List<Masters_FallingSortPhraseCard> activeCards = new List<Masters_FallingSortPhraseCard>();
    protected Dictionary<Masters_FallingSortPhraseCard, Masters_FallingSortBin> cardTargetBins = new Dictionary<Masters_FallingSortPhraseCard, Masters_FallingSortBin>();

    protected List<string> recentlySpawnedExpressions = new List<string>();
    protected Masters_Unit8_FallingSortCategory lastSpawnedCategory = (Masters_Unit8_FallingSortCategory)(-1);
    ...
    /// <summary>
    /// Sets up the active bins for this game variant. Override in subclasses
    /// to change how many bins are active and what categories they represent.
    /// Default implementation: 2 bins (FORMAL / INFORMAL) for Unit 1.
    /// </summary>
            ...
            if (allBins.Length <= 2) {
                for (int i = 0; i < allBins.Length; i++) {
                    if (allBins[i] != null) {
                        ...
                        allBins[i].SetUnit8Category((i == 0) ? Masters_Unit8_FallingSortCategory.Ask : Masters_Unit8_FallingSortCategory.State);
                    }
                }
            } else {
                // Use 1st bin (index 0) and last bin (index Length - 1) as they are visually at opposite screen edges
                Masters_FallingSortBin firstBin = allBins[0];
                Masters_FallingSortBin lastBin = allBins[allBins.Length - 1];

                for (int i = 0; i < allBins.Length; i++) {
                    if (allBins[i] != null) {
                        if (allBins[i] == firstBin) {
                            activeBins.Add(allBins[i]);
                            allBins[i].gameObject.SetActive(true);
                            allBins[i].SetUnit8Category(Masters_Unit8_FallingSortCategory.Ask);
                        } else if (allBins[i] == lastBin) {
                            activeBins.Add(allBins[i]);
                            allBins[i].gameObject.SetActive(true);
                            allBins[i].SetUnit8Category(Masters_Unit8_FallingSortCategory.State);
                        } else {
                            allBins[i].gameObject.SetActive(false);
                        }
                    }
                }
            }
    ...
    protected virtual void SpawnRandomCard() {
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            // 3. Prevent more than 2 of the exact same category in a row
    ...
    protected void HandleCardDragEnded(Masters_FallingSortPhraseCard card) {
    ...
    public void SetSortPuzzles(SortPuzzle[] puzzles) {
        sortPuzzleArray = puzzles;
    }
}
