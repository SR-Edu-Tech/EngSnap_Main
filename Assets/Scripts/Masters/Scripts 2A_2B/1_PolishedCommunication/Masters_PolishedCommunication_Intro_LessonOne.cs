...
/// <summary>
/// Core Intro controller for Unit 1: Polished Communication (Book 2A Reference Base).
/// Manages sequential presentation of concept cards/street graphics, narrator audio,
/// and dynamic runtime duplication/auto-scrolling of Formal vs. Informal master reference sentences.
/// </summary>
public class Masters_PolishedCommunication_Intro_LessonOne : Masters_Lesson {
    ...
    private IEnumerator AutoScrollCoroutine(ScrollRect scrollRect) {
        // Wait until the scroll rect is actually active in hierarchy before trying to scroll or rebuild layout
        ...
        // Force layout update before scrolling so Unity calculates total height properly
        Canvas.ForceUpdateCanvases();
        LayoutRebuilder.ForceRebuildLayoutImmediate(scrollRect.content);

        // Disable inertia while auto-scrolling so UI physics don't interfere
        ...
        while (true) {
            ...
            while (elapsed < autoScrollDuration) {
                ...
                // Smoothly lerp from top (1f) to bottom (0f) with easing
                float easedT = Mathf.SmoothStep(0f, 1f, t);
                scrollRect.verticalNormalizedPosition = Mathf.Lerp(1f, 0f, easedT);
                yield return null;
            }

            scrollRect.verticalNormalizedPosition = 0f; // Ensure at bottom
            yield return new WaitForSeconds(3f); // Pause at bottom

            // Smoothly scroll back to top over 3 seconds
            elapsed = 0f;
            while (elapsed < 3f) {
                if (scrollRect == null) yield break;
                elapsed += Time.deltaTime;
            ...
            yield return new WaitForSeconds(2f); // Pause at top before looping
        }
    }
}
