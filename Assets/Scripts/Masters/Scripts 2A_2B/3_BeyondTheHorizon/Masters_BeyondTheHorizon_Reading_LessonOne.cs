...
/// <summary>
/// Core Reading 1 controller for Unit 3: Beyond the Horizon (Book 2A).
/// R01 — Pick the Right Phrase (in-context across 12 rounds).
/// Displays a situation prompt + [kind] hint, and offers 4 chip options (verbatim correct chip + 3 distractors).
/// Pure reading focus: No phrase voiceovers played on selection.
/// </summary>
public class Masters_BeyondTheHorizon_Reading_LessonOne : Masters_Lesson {
...
#pragma warning restore 0414
    ...
    private void AutoFindExpressionTMP() {
        Transform exprTransform = transform.Find("Expression");
        ...
        if (exprTransform == null) exprTransform = FindChildRecursive(transform, "ShownExpression");

        if (exprTransform != null) {
            shownExpressionTMP = exprTransform.GetComponent<TextMeshProUGUI>();
            if (shownExpressionTMP == null) shownExpressionTMP = exprTransform.GetComponentInChildren<TextMeshProUGUI>(true);
        }

        if (shownExpressionTMP == null) {
            TextMeshProUGUI[] tmps = GetComponentsInChildren<TextMeshProUGUI>(true);
            foreach (var t in tmps) {
                if (t.name.ToLower().Contains("expression") || t.name.ToLower().Contains("situation") || t.name.ToLower().Contains("prompt")) {
                    shownExpressionTMP = t;
                    break;
                }
            }
        }
    ...
    public void SetReadingData(SituationRoundData[] data) {
        rounds = data;
    }
}
