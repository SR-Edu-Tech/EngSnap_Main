...
public enum Masters_Unit2_ClarificationJob {
    AskToRepeat = 0,
    ExplainAgain = 1,
    ReasonAndAsk = 2,
    AskPermission = 3,
...
/// <summary>
/// Core Listening 1 controller for Unit 2: Clear Confusion (Book 2A).
/// Audio-to-job recognition across 10 rounds: student listens to a voiced clarification phrase
/// and matches it to 1 of 5 job chips (`ASK TO REPEAT`, `EXPLAIN AGAIN`, `REASON + ASK`, `ASK PERMISSION`, `SIGNAL POLITELY`).
/// Leverages slow & repeat toggle architecture from Unit 1 Listening (`1_PolishedCommunication`).
/// </summary>
public class Masters_ClearConfusion_Listening_LessonOne : Masters_Lesson {

    [System.Serializable]
    public class ClarificationQuestionData {
        public AudioClip expressionAudio;
        public AudioClip slowAudio;
        public string expressionText;
        public Masters_Unit2_ClarificationJob correctJob;
    }

    [Header("Listening L1 Data")]
    [SerializeField] private ClarificationQuestionData[] questions;
    [SerializeField] private Button[] optionButtons; // 0: ASK TO REPEAT, 1: EXPLAIN AGAIN, 2: REASON + ASK, 3: ASK PERMISSION, 4: SIGNAL POLITELY
    ...
    private void ConfigureOptionButtons() {
        ...
        string[] labels = { "ASK TO REPEAT", "EXPLAIN AGAIN", "REASON + ASK", "ASK PERMISSION", "SIGNAL POLITELY" };
        for (int i = 0; i < optionButtons.Length; i++) {
            if (optionButtons[i] != null) {
    ...
    private IEnumerator InitializeLessonRoutine() {
        if (Masters_AudioManager.Instance != null) {
            ...
            yield return new WaitForSeconds(narratorSpeech != null ? narratorSpeech.length + 0.5f : 3f);
        }
    ...
    private void PlayCurrentQuestionAudio() {
        ...
        ClarificationQuestionData question = questions[currentQuestionIndex];
    ...
    private void OnOptionSelected(int buttonIndex) {
        ...
        ClarificationQuestionData q = questions[currentQuestionIndex];
        if (q == null) return;

        Masters_Unit2_ClarificationJob selectedJob = (Masters_Unit2_ClarificationJob)buttonIndex;

        if (selectedJob == q.correctJob) {
            isAnswering = true;
            if (audioCoroutine != null) {
                StopCoroutine(audioCoroutine);
                audioCoroutine = null;
    ...
    private void OnAllQuestionsCompleted() {
        ...
        if (correctScore >= passThreshold) {
            ...
            // Replay option if threshold not met
            currentQuestionIndex = 0;
            correctScore = 0;
            LoadQuestion(0);
        }
