...
namespace EngSnap.Phonics2.Unit5
{
    public class VowelTeamsController : MonoBehaviour
    {
        ...
        private int currentWordWallFamilyIndex = 0;
        private System.Collections.Generic.HashSet<int> exploredCardsInCurrentBatch = new System.Collections.Generic.HashSet<int>();
        ...
        public void StartWordWallPhase()
        {
            if (walkingPanel != null) walkingPanel.SetActive(false);
            if (spottingPanel != null) spottingPanel.SetActive(false);
            if (wordWallPanel != null) wordWallPanel.SetActive(true);

            currentWordWallFamilyIndex = 0;
            LoadWordWallFamilyBatch(0);
        }

        private void LoadWordWallFamilyBatch(int familyIndex)
        {
            currentWordWallFamilyIndex = familyIndex;
            exploredCardsInCurrentBatch.Clear();

            string[] currentWords = GetWordsForFamily(familyIndex);
            string description = GetDescriptionForFamily(familyIndex);
            ...
            if (wordWallButtons != null)
            {
                for (int i = 0; i < wordWallButtons.Length; i++)
                {
                    if (wordWallButtons[i] != null)
                    {
                        ...
                        {
                            if (wordWallTexts != null && i < wordWallTexts.Length && wordWallTexts[i] != null)
                            {
                                wordWallTexts[i].text = currentWords[i];
                            }

                            if (wordWallImages != null && i < wordWallImages.Length && wordWallImages[i] != null)
                            {
                                Sprite sprite = GetSpriteForWord(currentWords[i], familyIndex, i);
                                if (sprite != null)
                                {
                                    wordWallImages[i].sprite = sprite;
                                    wordWallImages[i].gameObject.SetActive(true);
                                }
                                else
                                {
                                    wordWallImages[i].gameObject.SetActive(false);
                                }
                            }

                            wordWallButtons[i].transform.localScale = Vector3.one;
                            wordWallButtons[i].gameObject.SetActive(true);
                        }
                        else
                        {
                            wordWallButtons[i].gameObject.SetActive(false);
                        }
                    }
                }
            }

            float progress = 0.75f + ((familyIndex + 1) / (float)GetTotalWordWallFamilies()) * 0.2f;
            UpdateProgressUI(progress);
        }

        private void OnWordWallCardClicked(int slotIndex)
        {
            if (isTransitioning) return;

            string[] currentWords = GetWordsForFamily(currentWordWallFamilyIndex);
            if (slotIndex < 0 || slotIndex >= currentWords.Length) return;

            PlaySFX(activityData.starPopSfx);
            if (wordWallButtons != null && slotIndex < wordWallButtons.Length && wordWallButtons[slotIndex] != null)
            {
                TriggerWiggle(wordWallButtons[slotIndex].GetComponent<RectTransform>());
            }

            string word = currentWords[slotIndex];
            SetDialogue($"Vowel team sound: '{word.ToUpper()}'!");

            AudioClip clip = GetAudioClipForWord(word, currentWordWallFamilyIndex, slotIndex);
            if (clip != null)
            {
                PlayVoiceClipNonBlocking(clip);
            }

            exploredCardsInCurrentBatch.Add(slotIndex);

            // Once the kid taps all words in the current batch, automatically advance to the next batch!
            if (exploredCardsInCurrentBatch.Count >= currentWords.Length)
            {
                StartCoroutine(AdvanceWordWallBatchSequence());
            }
        }

        private IEnumerator AdvanceWordWallBatchSequence()
        {
            isTransitioning = true;
            PlaySFX(activityData.correctChimeSfx);
            TriggerWiggleStarMeter();
            ...
            currentWordWallFamilyIndex++;
            if (currentWordWallFamilyIndex < GetTotalWordWallFamilies())
            {
                SetDialogue("Awesome! Here comes the next word family...");
                yield return new WaitForSeconds(0.5f);
                LoadWordWallFamilyBatch(currentWordWallFamilyIndex);
                isTransitioning = false;
            }
            else
            {
        ...
        private int GetTotalWordWallFamilies()
        {
            if (activityData != null && activityData.wordWallFamilies != null && activityData.wordWallFamilies.Length > 0)
            {
                return activityData.wordWallFamilies.Length;
            }
            return 4;
        }

        private string[] GetWordsForFamily(int familyIndex)
        {
            if (activityData != null && activityData.wordWallFamilies != null && familyIndex < activityData.wordWallFamilies.Length)
            {
                if (activityData.wordWallFamilies[familyIndex].words != null)
                {
                    return activityData.wordWallFamilies[familyIndex].words;
                }
            }

            switch (familyIndex)
            {
                case 0: return new string[] { "eat", "beat", "beak", "leak", "weak" };
                case 1: return new string[] { "sheep", "feet", "seat", "meat", "mean", "bean", "seal", "meal", "leaf" };
                case 2: return new string[] { "boat", "coat", "toast", "road" };
                case 3: return new string[] { "rose", "cone", "rope", "stone", "phone", "alone" };
                default: return new string[] { "eat", "beat", "beak", "leak", "weak" };
            }
        }

        private string GetDescriptionForFamily(int familyIndex)
        {
            if (activityData != null && activityData.wordWallFamilies != null && familyIndex < activityData.wordWallFamilies.Length)
            {
                if (!string.IsNullOrEmpty(activityData.wordWallFamilies[familyIndex].familyDescription))
                {
                    return activityData.wordWallFamilies[familyIndex].familyDescription;
                }
            }

            switch (familyIndex)
            {
                case 0: return "Leo: Long E team: -eat and -eak words! Tap every word!";
                case 1: return "Leo: Long E team: ee and ea words! Tap every word!";
                case 2: return "Leo: Long O team: oa words! Tap every word!";
                case 3: return "Leo: Magic E reminder: Long O words can also use magic e!";
                default: return "Leo: Tap every word to hear its sound!";
            }
        }

        private AudioClip GetAudioClipForWord(string word, int familyIndex, int slotIndex)
        {
            if (activityData != null && activityData.wordWallFamilies != null && familyIndex < activityData.wordWallFamilies.Length)
            {
                VowelTeamWordFamily fam = activityData.wordWallFamilies[familyIndex];
                if (fam.audioClips != null && slotIndex < fam.audioClips.Length && fam.audioClips[slotIndex] != null)
                {
                    return fam.audioClips[slotIndex];
                }
            }

            if (activityData != null && activityData.vowelTeamsWordWallList != null && activityData.vowelTeamsWordWallClips != null)
            {
                for (int i = 0; i < activityData.vowelTeamsWordWallList.Length; i++)
                {
                    if (string.Equals(activityData.vowelTeamsWordWallList[i], word, System.StringComparison.OrdinalIgnoreCase))
                    {
                        if (i < activityData.vowelTeamsWordWallClips.Length)
                        {
                            return activityData.vowelTeamsWordWallClips[i];
                        }
                    }
                }
            }
        ...
        private Sprite GetSpriteForWord(string word, int familyIndex, int slotIndex)
        {
            if (activityData != null && activityData.wordWallFamilies != null && familyIndex < activityData.wordWallFamilies.Length)
            {
                VowelTeamWordFamily fam = activityData.wordWallFamilies[familyIndex];
                if (fam.sprites != null && slotIndex < fam.sprites.Length && fam.sprites[slotIndex] != null)
                {
                    return fam.sprites[slotIndex];
                }
            }

            if (activityData != null && activityData.vowelTeamsWordWallList != null && activityData.vowelTeamsWordWallSprites != null)
            {
                for (int i = 0; i < activityData.vowelTeamsWordWallList.Length; i++)
                {
                    if (string.Equals(activityData.vowelTeamsWordWallList[i], word, System.StringComparison.OrdinalIgnoreCase))
                    {
                        if (i < activityData.vowelTeamsWordWallSprites.Length)
                        {
                            return activityData.vowelTeamsWordWallSprites[i];
                        }
                    }
                }
            }
