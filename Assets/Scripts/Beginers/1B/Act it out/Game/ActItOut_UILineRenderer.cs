...
namespace Beginners.ActItOut
{
    ...
    public class ActItOut_UILineRenderer : Graphic
    {
        ...
        public void RefreshCanvas()
        {
            ...
            Debug.Log($"[ActItOut_UILineRenderer] RefreshCanvas → rootCanvas={(_rootCanvas ? _rootCanvas.name : "NULL")}  renderMode={(_rootCanvas ? _rootCanvas.renderMode.ToString() : "?")}  cam={(_cam ? _cam.name : "NULL")}");
        }
        ...
        public void SetWorldPoints(Vector3 worldFrom, Vector3 worldTo)
        {
            ...
            _hasPoints = true;
            Debug.Log($"[ActItOut_UILineRenderer] SetWorldPoints  localFrom={_localFrom}  localTo={_localTo}  rectSize={rectTransform.rect.size}");
            SetVerticesDirty();
        }
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            Debug.Log($"[ActItOut_UILineRenderer] OnPopulateMesh  from={from}  to={to}  perp={perp}  lineWidth={lineWidth}  color={color}");
        }
