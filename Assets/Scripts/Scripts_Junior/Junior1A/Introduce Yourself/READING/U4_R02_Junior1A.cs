...
public class U4_R02_Junior1A : MonoBehaviour, Interfaces_Junior1A
{
    ...
    void OnEnable()
    {
        ...
        _tab1P.GetComponent<CanvasGroup>().alpha = 0;
        _tab2P.GetComponent<CanvasGroup>().alpha = 0;
    ...
    IEnumerator OnTab()
    {
        ...
        {
            for (int i = 0; i < _tab1P.transform.childCount; i++)
            {
            ...
            for (int i = 0; i < _tab1P.transform.childCount; i++) _tab1P.transform.GetChild(i).GetComponent<Button>().interactable = true;
        }
        else
        {
            int _currentSpriteIndex = 0;

            _tab2P.transform.GetChild(_tab2P.transform.childCount - 1).GetComponent<Image>().sprite = _tab2P.transform.GetChild(_currentSpriteIndex).GetChild(0).GetChild(0).GetComponent<Image>().sprite;
            ...
            for (int i = 0; i < _tab2P.transform.childCount - 1; i++)
            {
                ...
                _tab2P.transform.GetChild(_tab2P.transform.childCount - 1).GetComponent<Image>().sprite = _tab2P.transform.GetChild(_currentSpriteIndex).GetChild(0).GetChild(0).GetComponent<Image>().sprite;
                _tab2P.transform.GetChild(i).GetComponent<Button>().onClick.Invoke();
                _currentSpriteIndex++;

                yield return new WaitForSeconds(_audioSource.clip.length);
            }
            for (int i = 0; i < _tab2P.transform.childCount - 1; i++) _tab2P.transform.GetChild(i).GetComponent<Button>().interactable = true;
        }
    }
    public void OnSpeaker(Image speakerIcon)
    {
        if (_currentSpeakerIcon)
        {
            _currentSpeakerIcon.color = Color.white;
            if (_currentAudioClipIndex < 5) _currentSpeakerIcon.transform.parent.GetComponent<Image>().color = Color.white;
        }
    ...
    IEnumerator PlayAudioIndex()
    {
        ...
        if (_currentAudioClipIndex < 5) _currentSpeakerIcon.transform.parent.GetComponent<Image>().color = new Color(1, 0.8500008f, 0.5518868f, 1);
