...
//
public class TopicSelectionManager_junior : MonoBehaviour
{
    [Header("Panels")]
    ...
    [Header("Topics")]
    public List<WordTopicData_junior> topics = new List<WordTopicData_junior>();

    [Header("UI References")]
    public Transform       topicListParent;
    public TopicItemUI_junior     topicItemPrefab;
    public TextMeshProUGUI headerText;

    [Header("Game")]
    public WordSearchManager_junior wordSearchManager;

    private List<TopicItemUI_junior> topicItems  = new List<TopicItemUI_junior>();
    private WordTopicData_junior     currentTopic;

    static string DoneKey(WordTopicData_junior d) => "wsdone_" + d.topicName;
    ...
    void BuildTopicList()
    {
        ...
        foreach (WordTopicData_junior topic in topics)
        {
            TopicItemUI_junior item = Instantiate(topicItemPrefab, topicListParent);
            item.Init(topic, this);
            topicItems.Add(item);
        }
    }

    public void SelectTopic(WordTopicData_junior topic)
    {
    ...
    public void ResetTopic(WordTopicData_junior topic)
    {
    ...
    public static bool IsTopicDone(WordTopicData_junior topic)
        => topic != null && PlayerPrefs.GetInt(DoneKey(topic), 0) == 1;

    void MarkTopicDone(WordTopicData_junior topic)
    {
        if (topic == null) return;
