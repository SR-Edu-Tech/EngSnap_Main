...
public class WordSearchManager_junior : MonoBehaviour
{
    [Header("Grid")]
    ...
    public WordSearchCell_junior cellPrefab;
    ...
    public WordItemUI_junior wordItemPrefab;
    ...
    private WordSearchCell_junior[,] grid;
    private List<WordSearchCell_junior> selection  = new List<WordSearchCell_junior>();
    private Vector2Int selDir;
    private bool       isDragging;

    private Dictionary<string, WordItemUI_junior> wordUIMap  = new Dictionary<string, WordItemUI_junior>();
    private HashSet<string>                foundWords = new HashSet<string>();
    private Image dragPill;

    private HashSet<WordSearchCell_junior> scaledCells = new HashSet<WordSearchCell_junior>();
    ...
    // Load new topic
    public void LoadTopic(WordTopicData_junior topic)
    {
    ...
    void Update()
    {
        ...
        WordSearchCell_junior hit = GetCellAtScreenPos(screenPos);
    ...
    void CreateWordListUI()
    {
        ...
        {
            WordItemUI_junior item = Instantiate(wordItemPrefab, wordListParent);
            item.Init(word);
            wordUIMap[word] = item;
        }
    }
    ...
    void BuildGrid()
    {
        ...
        for (int i = gridParent.childCount - 1; i >= 0; i--)
        {
            ...
            if (t.GetComponent<WordSearchCell_junior>() != null) Destroy(t.gameObject);
        }
        ...
        grid = new WordSearchCell_junior[rows, cols];
        for (int r = 0; r < rows; r++)
            for (int c2 = 0; c2 < cols; c2++)
            {
                WordSearchCell_junior cell = Instantiate(cellPrefab, gridParent);
    ...
    // Find word cells
    List<WordSearchCell_junior> FindWordCells(string word)
    {
        ...
        for (int r = 0; r < rows; r++)
            for (int c = 0; c < cols; c++)
                ...
                {
                    List<WordSearchCell_junior> cells = new List<WordSearchCell_junior>();
                    bool ok = true;
                    for (int i = 0; i < word.Length; i++)
                    {
    ...
    public void StartSelection(WordSearchCell_junior cell)
    {
    ...
    public void ContinueSelection(WordSearchCell_junior cell)
    {
        ...
        {
            WordSearchCell_junior removed = selection[selection.Count - 1];
        ...
        WordSearchCell_junior first = selection[0];
        WordSearchCell_junior last  = selection[selection.Count - 1];
    ...
    void SetCellDragScale(WordSearchCell_junior cell, bool enlarged)
    {
    ...
    WordSearchCell_junior GetCellAtScreenPos(Vector2 screenPos)
    {
        if (grid == null) return null;

        float bestDist = float.MaxValue;
        WordSearchCell_junior bestCell = null;
        ...
        for (int r = rMin; r <= rMax; r++)
            ...
            {
                WordSearchCell_junior cell = grid[r, c];
    ...
    Vector2 GetCellScreenCenter(WordSearchCell_junior cell)
    {
    ...
    float GetCellScreenSize(WordSearchCell_junior cell)
    {
        Vector3[] corners = new Vector3[4];
        cell.GetComponent<RectTransform>().GetWorldCorners(corners);
    ...
    Image SpawnPill(List<WordSearchCell_junior> cells, Color color)
    {
        if (cells == null || cells.Count == 0 || pillPrefab == null || pillParent == null)
            return null;

        WordSearchCell_junior first = cells[0];
        WordSearchCell_junior last  = cells[cells.Count - 1];
    ...
    Vector3 GetCellWorldCenter(WordSearchCell_junior cell)
    {
    ...
    float GetCellWorldSize(WordSearchCell_junior cell)
    {
        Vector3[] c = new Vector3[4];
        cell.GetComponent<RectTransform>().GetWorldCorners(c);
