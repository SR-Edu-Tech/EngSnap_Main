...
public class ListeningScreens_S3A : MonoBehaviour
{
    [Header("Audio")]
    ...
    void ResetUIState()
    {
        ...
        foreach (var card in cards)
        {
            ...
            CanvasGroup textCG = card.text.GetComponent<CanvasGroup>();
            if (textCG == null) textCG = card.text.gameObject.AddComponent<CanvasGroup>();
            textCG.alpha = 0f;
            ...
            if (card.extraText != null)
            {
                ...
                CanvasGroup extraCG = card.extraText.GetComponent<CanvasGroup>();
                if (extraCG == null) extraCG = card.extraText.gameObject.AddComponent<CanvasGroup>();
                extraCG.alpha = 0f;
            }
    ...
    void SetPlaying(Card card, bool isManual)
    {
        ...
        card.button.transform.localScale = Vector3.one * 1.1f;
