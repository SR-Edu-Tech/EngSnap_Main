...
[System.Serializable]
public class GuessBeginningBlendQuestion
{
    [Tooltip("The full correct word, e.g. 'brain'")]
    public string wordText;

    [Tooltip("The text representation with gap, e.g. '__ain'.")]
    public string gappedWordText;

    [Tooltip("The correct beginning blend, e.g. 'br'")]
    public string correctBlend;
    ...
    [Tooltip("The spelling choices shown for this question (usually 4 choices)")]
    public string[] options;
}

[System.Serializable]
public class GuessBeginningBlendOptionUI
{
...
public class GuessBeginningBlend_P_Senior : MonoBehaviour
{
    [Header("Unit Complete Mascot Audio")]
    public AudioClip unitCompleteAudio;
    [Header("Gameplay Config")]
    public List<GuessBeginningBlendQuestion> questions = new List<GuessBeginningBlendQuestion>();

    [Header("UI Components - Options Tray")]
    [Tooltip("The UI elements mapped for each option slot in the tray (usually 4 or 6)")]
    public GuessBeginningBlendOptionUI[] optionUIElements;
    ...
    public void ResetToStart()
    {
        ...
        _audioCoroutine = StartCoroutine(IntroAndLoadFlow());
    }

    private IEnumerator IntroAndLoadFlow()
    {
        _canTap = false;

        // 1. Play general screen intro audio if configured
        if (mascotAudioSource != null && introScreenAudio != null)
        {
            mascotAudioSource.Stop();
            mascotAudioSource.clip = introScreenAudio;
            ...
            // Run talk animation
            yield return StartCoroutine(MascotTalkAnimation(introScreenAudio.length));
    ...
    private void LoadQuestion(int index)
    {
        ...
        if (questions == null || questions.Count == 0)
        {
            Debug.LogWarning("[GuessBeginningBlend] No questions configured!");
            return;
        }
        ...
        {
            instructionLabel.text = "Tap the correct beginning blend to complete the word.";
        }
        ...
        {
            currentChoices = new string[] { "br", "cl", "bl", "cr" }; // Fallback options
        }
        ...
        for (int i = 0; i < optionUIElements.Length; i++)
        {
            ...
            if (i < currentChoices.Length)
            {
                ...
                // Apply option background colors
        ...
        _canTap = false;
        if (mascotCharacter != null && mascotCharacter.localScale.x < 0.1f)
        {
            mascotCharacter.localScale = Vector3.zero;
            LeanTween.cancel(mascotCharacter.gameObject);
            LeanTween.scale(mascotCharacter.gameObject, _originalMascotScale, 0.45f)
                .setEase(LeanTweenType.easeOutBack)
                .setOnComplete(() => PlayNormalWordAudio(data));
        }
        else
        {
    ...
    private void PlayNormalWordAudio(GuessBeginningBlendQuestion data)
    {
        if (data == null) return;
    ...
    private IEnumerator PlayAudioSequence(AudioClip clip, System.Action onComplete)
    {
        ...
        {
            yield return new WaitForSeconds(1.5f); // Simulated delay if audio is missing
        }
    ...
    private void HandleCorrectChoice(int index)
    {
        ...
        if (wordTextLabel != null)
        {
            string spellingColored = $"<color={correctColorHex}>{data.correctBlend}</color>";
    ...
    private void HandleIncorrectChoice(int index, string incorrectSpelling)
    {
        ...
        _audioCoroutine = StartCoroutine(PlayAudioSequence(repeatClip, () => {
            _canTap = true;
            if (instructionLabel != null)
            {
                instructionLabel.text = "Tap the correct beginning blend to complete the word.";
    ...
    {
        Debug.Log("[GuessBeginningBlend] Completed all questions!");
    ...
    {
        questions = new List<GuessBeginningBlendQuestion>
        {
            new GuessBeginningBlendQuestion { wordText = "brain", gappedWordText = "__ain", correctBlend = "br", options = new string[] { "br", "cl", "bl", "cr" } },
            new GuessBeginningBlendQuestion { wordText = "cloud", gappedWordText = "__oud", correctBlend = "cl", options = new string[] { "cl", "bl", "cr", "gl" } },
            new GuessBeginningBlendQuestion { wordText = "block", gappedWordText = "__ock", correctBlend = "bl", options = new string[] { "bl", "br", "cl", "fl" } },
            new GuessBeginningBlendQuestion { wordText = "crayon", gappedWordText = "__ayon", correctBlend = "cr", options = new string[] { "cr", "cl", "br", "gr" } },
            new GuessBeginningBlendQuestion { wordText = "clown", gappedWordText = "__own", correctBlend = "cl", options = new string[] { "cl", "cr", "gl", "fl" } },
            new GuessBeginningBlendQuestion { wordText = "crab", gappedWordText = "__ab", correctBlend = "cr", options = new string[] { "cr", "cl", "br", "fr" } },
            new GuessBeginningBlendQuestion { wordText = "bread", gappedWordText = "__ead", correctBlend = "br", options = new string[] { "br", "bl", "cr", "gr" } },
            new GuessBeginningBlendQuestion { wordText = "clock", gappedWordText = "__ock", correctBlend = "cl", options = new string[] { "cl", "cr", "bl", "gl" } },
            new GuessBeginningBlendQuestion { wordText = "frog", gappedWordText = "__og", correctBlend = "fr", options = new string[] { "fr", "fl", "gr", "br" } },
            new GuessBeginningBlendQuestion { wordText = "glue", gappedWordText = "__ue", correctBlend = "gl", options = new string[] { "gl", "gr", "cl", "fl" } },
            new GuessBeginningBlendQuestion { wordText = "flower", gappedWordText = "__ower", correctBlend = "fl", options = new string[] { "fl", "fr", "gl", "bl" } },
            new GuessBeginningBlendQuestion { wordText = "grapes", gappedWordText = "__apes", correctBlend = "gr", options = new string[] { "gr", "gl", "cr", "br" } },
            new GuessBeginningBlendQuestion { wordText = "frame", gappedWordText = "__ame", correctBlend = "fr", options = new string[] { "fr", "fl", "gr", "cr" } },
            new GuessBeginningBlendQuestion { wordText = "glass", gappedWordText = "__ass", correctBlend = "gl", options = new string[] { "gl", "gr", "cl", "fl" } },
            new GuessBeginningBlendQuestion { wordText = "fruits", gappedWordText = "__uits", correctBlend = "fr", options = new string[] { "fr", "fl", "gr", "br" } },
            new GuessBeginningBlendQuestion { wordText = "grass", gappedWordText = "__ass", correctBlend = "gr", options = new string[] { "gr", "gl", "fr", "cr" } }
        };
        UpdateOptionIndexes();
    }
}
