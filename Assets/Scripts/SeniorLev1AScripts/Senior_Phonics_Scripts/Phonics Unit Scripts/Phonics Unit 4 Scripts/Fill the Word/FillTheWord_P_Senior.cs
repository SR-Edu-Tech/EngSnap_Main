...
[System.Serializable]
public class FillTheWordQuestion
{
    [Tooltip("The full correct word, e.g. 'fountain'")]
    public string wordText;

    [Tooltip("The text representation with gap, e.g. 'f__ntain'. The script will search for '__' to replace with feedback.")]
    public string gappedWordText;

    [Tooltip("The correct spelling to fill the gap, e.g. 'ou'")]
    ...
    [Tooltip("The spelling choices shown for this question (usually 4 choices like oi, oy, ou, ow). If empty, the default choices will be used.")]
    public string[] options;
}

[System.Serializable]
public class FillTheWordOptionUI
{
...
public class FillTheWord_P_Senior : MonoBehaviour
{
    ...
    public List<FillTheWordQuestion> questions = new List<FillTheWordQuestion>();

    [Header("Default Option Options (Used if question options are empty)")]
    public string[] defaultOptions = new string[] { "oi", "oy", "ou", "ow" };

    [Header("UI Components - Options Tray")]
    [Tooltip("The UI elements mapped for each option slot in the tray (usually 4)")]
    public FillTheWordOptionUI[] optionUIElements;

    [Header("UI Colors - Option Backgrounds")]
    [Tooltip("The default card background colors in order (e.g. Green, Yellow, Blue, Pink)")]
    ...
    private void Awake()
    {
        // Robust check to ensure defaultOptions is populated (prevents inspector override to size 0 at runtime)
        ...
        if (!defaultOptionsValid)
        {
            defaultOptions = new string[] { "oi", "oy", "ou", "ow" };
        }
    ...
    private string[] GetQuestionChoices(FillTheWordQuestion data)
    {
    ...
    private void LoadQuestion(int index)
    {
        ...
        {
            Debug.LogWarning("[FillTheWord] No questions configured!");
            return;
        }
        ...
        {
            instructionLabel.text = "Tap the letters to fill the word.";
        }

        // 3. Setup Option Cards
        string[] currentChoices = GetQuestionChoices(data);
        Debug.Log($"[FillTheWord] LoadQuestion index {index}. Word: '{data.wordText}', Gapped: '{data.gappedWordText}'. spelling choices: {string.Join(", ", currentChoices)}");
        ...
        for (int i = 0; i < optionUIElements.Length; i++)
        {
            ...
            if (opt == null)
            {
                Debug.LogWarning($"[FillTheWord] optionUIElements[{i}] is null in the Inspector array!");
                continue;
            }

            var containerObj = opt.container != null ? opt.container : (opt.cardButton != null ? opt.cardButton.gameObject : null);
            string containerId = containerObj != null ? containerObj.GetInstanceID().ToString() : "null";
            string buttonId = opt.cardButton != null ? opt.cardButton.GetInstanceID().ToString() : "null";
            string labelId = opt.spellingTextLabel != null ? opt.spellingTextLabel.GetInstanceID().ToString() : "null";
            
            Debug.Log($"[FillTheWord] Option {i}: container={(opt.container != null ? opt.container.name : "null")} (ID: {containerId}), button={(opt.cardButton != null ? opt.cardButton.name : "null")} (ID: {buttonId}), spellingTextLabel={(opt.spellingTextLabel != null ? opt.spellingTextLabel.name : "null")} (ID: {labelId})");
            ...
            if (i < currentChoices.Length)
            {
                if (containerObj != null)
                {
                    ...
                    Debug.Log($"[FillTheWord] Activated container object '{containerObj.name}' for option {i}");
                }
                else
                {
                    Debug.LogWarning($"[FillTheWord] Option {i} has no container or cardButton assigned!");
                }
                ...
                if (opt.spellingTextLabel != null)
                {
                    ...
                    Debug.Log($"[FillTheWord] Assigned option text '{currentChoices[i]}' to TextMeshProUGUI '{opt.spellingTextLabel.name}' (GameObject active: {opt.spellingTextLabel.gameObject.activeInHierarchy}, Text: '{opt.spellingTextLabel.text}')");
                }
                else
                {
                    Debug.LogError($"[FillTheWord] Option {i} does not have spellingTextLabel assigned in the Inspector!");
                }
    ...
    private void PlayNormalWordAudio(FillTheWordQuestion data)
    {
        if (data == null) return;
    ...
    {
        Debug.Log("[FillTheWord] Completed all questions!");
    ...
    private void SetupProgressDots()
    {
        ...
        {
            Debug.LogError("[FillTheWord] No progress dot prefab or template found!");
            return;
        }
    ...
    private void Reset()
    {
        // Setup the book's exact set by default
        if (questions == null || questions.Count == 0)
        {
            questions = new List<FillTheWordQuestion>()
            {
                new FillTheWordQuestion { wordText = "fountain", gappedWordText = "f__ntain", correctSpelling = "ou", correctOptionIndex = 2, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "oil", gappedWordText = "__l", correctSpelling = "oi", correctOptionIndex = 0, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "house", gappedWordText = "h__se", correctSpelling = "ou", correctOptionIndex = 2, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "annoy", gappedWordText = "ann__", correctSpelling = "oy", correctOptionIndex = 1, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "boy", gappedWordText = "b__", correctSpelling = "oy", correctOptionIndex = 1, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "bowl", gappedWordText = "b__l", correctSpelling = "ow", correctOptionIndex = 3, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "brown", gappedWordText = "br__n", correctSpelling = "ow", correctOptionIndex = 3, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "mouth", gappedWordText = "m__th", correctSpelling = "ou", correctOptionIndex = 2, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "flower", gappedWordText = "fl__er", correctSpelling = "ow", correctOptionIndex = 3, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "point", gappedWordText = "p__nt", correctSpelling = "oi", correctOptionIndex = 0, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "gown", gappedWordText = "g__n", correctSpelling = "ow", correctOptionIndex = 3, options = new string[] { "oi", "oy", "ou", "ow" } },
                new FillTheWordQuestion { wordText = "toy", gappedWordText = "t__", correctSpelling = "oy", correctOptionIndex = 1, options = new string[] { "oi", "oy", "ou", "ow" } }
            };
        }
    }

    private IEnumerator IntroAndStartFlow(FillTheWordQuestion data)
    {
        _canTap = false;
