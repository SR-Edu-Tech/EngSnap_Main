...
public class ListeningUnit12_S2A : MonoBehaviour
{
    [System.Serializable]
...
private void Start()
{
    ...
    // Keep the card panel's original X position.
    // Only move it vertically for the entrance animation.
    centeredX = cardsBG.anchoredPosition.x;

    cardsBG.anchoredPosition = new Vector2(
        centeredX,
        bgStartPos.y
    );
    ...
    IEnumerator StartSequence()
    {
        ...
        // BG SLIDE
        // Keep X centered and animate ONLY the Y position.
        LeanTween.moveY(cardsBG, bgTargetPos.y, 0.5f)
            .setEaseOutExpo();
