...
public class IntroScreenUnit1_S2A : MonoBehaviour
{
    [Header("Audio")]
    ...
    [Header("Extra UI Animations")]
    public RectTransform titleBG;      // Warm Greetings board
    public RectTransform contextBG;      // Big rounded rectangle
    ...
    void ResetUI()
    {
        ...
        titleBG.localScale = Vector3.zero;
        contextBG.localScale = Vector3.zero;
    ...
    IEnumerator IntroFlow()
    {
        ...
        // Character + Title BG together
StartCoroutine(PopCharacter());
yield return StartCoroutine(PopUI(titleBG));

// Title text
yield return StartCoroutine(AnimateTitle());

// Context BG appears
yield return StartCoroutine(PopUI(contextBG));
...
yield return StartCoroutine(FadeContent());
    ...
    IEnumerator PopUI(RectTransform target)
{
    if (target == null)
        yield break;
